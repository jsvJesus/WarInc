#include "NxApex.h"
#ifndef NX_USER_RENDER_VERTEXBUFFER_H
#define NX_USER_RENDER_VERTEXBUFFER_H
/*
	...
	*/
...
#if NX_APEX_LEGACY_WRITEBUFFER_API
	{
		...
		for (physx::PxU32 i = 0; i < NxRenderVertexSemantic::NUM_SEMANTICS; i++)
		{
			...
			{
				writeBuffer(semantic, semanticData.data, semanticData.stride, firstVertex, numVertices);
			}
		}
		for (physx::PxU32 i = 0; i < data.numModuleSpecificSemantics; i++)
		{
			const NxApexRenderSemanticData& semanticData = data.moduleSpecificSemanticData[i];
			if (semanticData.data)
			{
				writeBuffer(data.moduleId, i, semanticData.data, semanticData.stride, firstVertex, numVertices);
			}
		}
	}
#else
...
private:
	...
	virtual void writeBuffer(NxRenderVertexSemantic::Enum semantic, const void* srcData, physx::PxU32 srcStride, physx::PxU32 firstDestElement, physx::PxU32 numElements) = 0;
	virtual void writeBuffer(NxAuthObjTypeID moduleId, physx::PxU32 semantic, const void* srcData, physx::PxU32 srcStride, physx::PxU32 firstDestElement, physx::PxU32 numElements) = 0;
#endif
};

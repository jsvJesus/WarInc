...
#ifndef PX_FOUNDATION_PXMAT33_H
#define PX_FOUNDATION_PXMAT33_H
...
public:
	//! Default constructor
	PX_CUDA_MEMBER PX_INLINE PxMat33()
	{}

	//! Construct from three base vectors
	PX_CUDA_MEMBER PxMat33(const PxVec3& col0, const PxVec3& col1, const PxVec3& col2)
		: column0(col0), column1(col1), column2(col2)
	{}

	//! Construct from float[9]
	PX_CUDA_MEMBER explicit PX_INLINE PxMat33(PxReal values[]):
	...
	PX_CUDA_MEMBER explicit PX_FORCE_INLINE PxMat33(const PxQuat& q)
	{
		const PxReal x = q.x;
		const PxReal y = q.y;
		const PxReal z = q.z;
	...
	//! Copy constructor
	PX_CUDA_MEMBER PX_INLINE PxMat33(const PxMat33& other)
		: column0(other.column0), column1(other.column1), column2(other.column2)
	{}

	//! Assignment operator
	PX_CUDA_MEMBER PX_FORCE_INLINE PxMat33& operator=(const PxMat33& other)
	{
	...
	PX_CUDA_MEMBER PX_INLINE static PxMat33 identity()
	{
		return getIdentity();
	}

	//! Set to zero matrix
	PX_CUDA_MEMBER PX_INLINE static PxMat33 zero()
	{
		return getZero();
	}

	//! Set to identity matrix
	PX_CUDA_MEMBER PX_INLINE static PxMat33 getIdentity()
	{
		return PxMat33(PxVec3(1,0,0), PxVec3(0,1,0), PxVec3(0,0,1));
	}

	//! Set to zero matrix
	PX_CUDA_MEMBER PX_INLINE static PxMat33 getZero()
	{
		return PxMat33(PxVec3(0.0f), PxVec3(0.0f), PxVec3(0.0f));
	}

	//! Construct from diagonal, off-diagonals are zero.
	PX_CUDA_MEMBER PX_INLINE static PxMat33 diagonal(const PxVec3& d)
	{
	...
	PX_CUDA_MEMBER PX_FORCE_INLINE PxMat33 getTranspose() const
	{
	...
	PX_CUDA_MEMBER PX_INLINE PxMat33 getInverse() const
	{
	...
	//! Get determinant
	PX_CUDA_MEMBER PX_INLINE PxReal getDeterminant() const
	{
		return column0.dot(column1.cross(column2));
	}

	//! Unary minus
	PX_CUDA_MEMBER PX_INLINE PxMat33 operator-() const
	{
		return PxMat33(-column0, -column1, -column2);
	}

	//! Add
	PX_CUDA_MEMBER PX_INLINE PxMat33 operator+(const PxMat33& other) const
	{
	...
	PX_CUDA_MEMBER PX_INLINE PxMat33 operator*(PxReal scalar) const
	{
	...
	PX_CUDA_MEMBER PX_INLINE PxVec3 operator*(const PxVec3& vec) const
	{
		return transform(vec);
	}

	//! Matrix multiplication
	PX_CUDA_MEMBER PX_FORCE_INLINE PxMat33 operator*(const PxMat33& other) const
	{
	...
	//! Equals-add
	PX_CUDA_MEMBER PX_INLINE PxMat33& operator+=(const PxMat33& other)
	{
		column0 += other.column0;
		column1 += other.column1;
		column2 += other.column2;
	...
	PX_CUDA_MEMBER PX_INLINE PxMat33& operator*=(PxReal scalar)
	{
		column0 *= scalar;
		column1 *= scalar;
		column2 *= scalar;
	...
	PX_CUDA_MEMBER PX_FORCE_INLINE PxVec3 transform(const PxVec3& other) const
	{
		return column0*other.x + column1*other.y + column2*other.z;
	}

	//! Transform vector by matrix transpose, v' = M^t*v
	PX_CUDA_MEMBER PX_INLINE PxVec3 transformTranspose(const PxVec3& other) const
	{
	...
	PX_CUDA_MEMBER PX_FORCE_INLINE			PxVec3& operator[](int num)			{return (&column0)[num];}
	PX_CUDA_MEMBER PX_FORCE_INLINE const	PxVec3& operator[](int num) const	{return (&column0)[num];}
...
PX_CUDA_MEMBER PX_INLINE PxQuat::PxQuat(const PxMat33& m)
{

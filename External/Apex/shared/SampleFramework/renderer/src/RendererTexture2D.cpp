...
static physx::PxU32 computeCompressedDimension(physx::PxU32 dimension)
{
	if(dimension < 4)
	{
		dimension = 4;
	...
	{
		physx::PxU32 mod = dimension % 4;
		if(mod) dimension += 4 - mod;
	}
	return dimension;
}

physx::PxU32 RendererTexture2D::computeImageByteSize(physx::PxU32 width, physx::PxU32 height, RendererTexture2D::Format format)
{
	physx::PxU32 size = 0;
	physx::PxU32 numPixels = width * height;
	switch(format)
	{
		case RendererTexture2D::FORMAT_B8G8R8A8:
			size = numPixels * sizeof(physx::PxU8) * 4;
			break;
		case RendererTexture2D::FORMAT_A8:
			size = numPixels * sizeof(physx::PxU8) * 1;
			break;
...
physx::PxU32 RendererTexture2D::getLevelDimension(physx::PxU32 dimension, physx::PxU32 level)
{
	dimension >>=level;
...
physx::PxU32 RendererTexture2D::getFormatNumBlocks(physx::PxU32 dimension, Format format)
{
	physx::PxU32 blockDimension = 0;
...
physx::PxU32 RendererTexture2D::getFormatBlockSize(Format format)
{
...
physx::PxU32 RendererTexture2D::getWidthInBlocks(void) const
{
	return getFormatNumBlocks(getWidth(), getFormat());
}

physx::PxU32 RendererTexture2D::getHeightInBlocks(void) const
{
	return getFormatNumBlocks(getHeight(), getFormat());
}

physx::PxU32 RendererTexture2D::getBlockSize(void) const
{

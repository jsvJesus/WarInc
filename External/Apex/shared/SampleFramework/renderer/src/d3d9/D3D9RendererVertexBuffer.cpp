...
static D3DDECLUSAGE getD3DUsage(RendererVertexBuffer::Semantic semantic, physx::PxU8 &usageIndex)
{
	...
	if(semantic >= RendererVertexBuffer::SEMANTIC_TEXCOORD0 && semantic <= RendererVertexBuffer::SEMANTIC_TEXCOORDMAX)
	{
		...
		usageIndex = (physx::PxU8)(semantic - RendererVertexBuffer::SEMANTIC_TEXCOORD0);
	}
	else
	{
		switch(semantic)
...
void D3D9RendererVertexBuffer::addVertexElements(physx::PxU32 streamIndex, std::vector<D3DVERTEXELEMENT9> &vertexElements) const
{
...
void *D3D9RendererVertexBuffer::lock(void)
{
	...
	if(m_d3dVertexBuffer)
	{
		const physx::PxU32 bufferSize = m_maxVertices * m_stride;
...
void D3D9RendererVertexBuffer::bind(physx::PxU32 streamID, physx::PxU32 firstVertex)
{
	prepareForRender();
	if(m_d3dVertexBuffer)
	{
...
void D3D9RendererVertexBuffer::unbind(physx::PxU32 streamID)
{

...
typedef std::vector< PxU32 > MeshIndexVector;
...
static PxI32 gSerializeFrame=1;

class MyMesh;

void validate(const MeshVertex &v)
{
	assert( PxIsFinite( v.mPos[0] ));
	assert( PxIsFinite( v.mPos[1] ));
	assert( PxIsFinite( v.mPos[2] ));
	assert( PxIsFinite( v.mNormal[0] ) );
	assert( PxIsFinite( v.mNormal[1] ) );
	assert( PxIsFinite( v.mNormal[2] ) );

	assert( PxIsFinite( v.mTexel1[0] ) );
	assert( PxIsFinite( v.mTexel1[1] ) );

	assert( PxIsFinite( v.mTexel2[0] ) );
	assert( PxIsFinite( v.mTexel2[1] ) );

	assert( PxIsFinite( v.mTexel3[0] ) );
	assert( PxIsFinite( v.mTexel3[1] ) );

	assert( PxIsFinite( v.mTexel4[0] ) );
	assert( PxIsFinite( v.mTexel4[1] ) );

	assert( PxIsFinite( v.mWeight[0] ) );
	assert( PxIsFinite( v.mWeight[1] ) );
	assert( PxIsFinite( v.mWeight[2] ) );
	assert( PxIsFinite( v.mWeight[3] ) );
	...
	PxF32 sum = v.mWeight[0] + v.mWeight[1] + v.mWeight[2] + v.mWeight[3];
	assert( sum >= 0 && sum <= 1.001f );
	assert( v.mBone[0] < 1024 );
	assert( v.mBone[1] < 1024 );
	assert( v.mBone[2] < 1024 );
	assert( v.mBone[3] < 1024 );

	assert( PxIsFinite(v.mTangent[0]));
	assert( PxIsFinite(v.mTangent[1]));
	assert( PxIsFinite(v.mTangent[2]));

	assert( PxIsFinite(v.mBiNormal[0]));
	assert( PxIsFinite(v.mBiNormal[1]));
	assert( PxIsFinite(v.mBiNormal[2]));

}

class MySubMesh : public SubMesh, public UserAllocated
{
public:
  MySubMesh(const char *mat,PxU32 vertexFlags)
  {
    mMaterialName = mat;
  ...
  bool isSame(const char *mat,PxU32 vertexFlags) const
  {
    bool ret = false;
    assert(mat);
  ...
  void gather(void)
  {
    mTriCount = (PxU32)mMyIndices.size()/3;
  ...
  void add(const MeshVertex &v1,const MeshVertex &v2,const MeshVertex &v3,VertexPool< MeshVertex > &vpool)
  {
    ...
    PxU32 i1 = vpool.GetVertex(v1);
    PxU32 i2 = vpool.GetVertex(v2);
    PxU32 i3 = vpool.GetVertex(v3);
...
class MyMesh : public Mesh, public UserAllocated
{
public:
  ...
  void getCurrent(const char *materialName,PxU32 vertexFlags)
  {
    ...
    if ( mCurrent == 0 || !mCurrent->isSame(materialName,vertexFlags) )
    {
      ...
      if ( mCurrent == 0 )
      {
        mCurrent = PX_NEW(MySubMesh)(materialName,vertexFlags);
  ...
  void gather(PxI32 bone_count)
  {
    mSubMeshes = 0;
    mSubMeshCount = 0;
    if ( !mMySubMeshes.empty() )
    {
      mSubMeshCount = (PxU32)mMySubMeshes.size();
      mSubMeshes    = &mMySubMeshes[0];
      for (PxU32 i=0; i<mSubMeshCount; i++)
      {
...
public:
  ...
  void gather(void)
  {
    mCollisionCount = (PxU32)mGeometries.size();
...
class MyMeshBuilder : public MeshBuilder, public UserAllocated
{
public:
  MyMeshBuilder(KeyValueIni *ini,const char *meshName,const void *data,PxU32 dlen,MeshImporter *mi,const char *options,MeshImportApplicationResource *appResource)
  {
    mINI = ini;
    gSerializeFrame++;
    mCurrentMesh = 0;
  ...
  virtual ~MyMeshBuilder(void)
  {
    ...
    if ( !mMyAnimations.empty() )
    {
      ...
      {
        MeshAnimation *a = (*i);
        for (PxI32 j=0; j<a->mTrackCount; j++)
        {
  ...
  void gather(void)
  {
    ...
    gatherMaterials();
    // todo..
    mMaterialCount = (PxU32)mMyMaterials.size();
    ...
    mMeshInstanceCount = (PxU32)mMyMeshInstances.size();
    ...
    mAnimationCount = (PxU32)mMyAnimations.size();
    if ( mAnimationCount )
    ...
    PxI32 bone_count = 0;
    mSkeletonCount = (PxU32)mMySkeletons.size();
    if ( mSkeletonCount )
    {
    ...
    mMeshCount = (PxU32)mMyMeshes.size();
    if ( mMeshCount )
    {
      PX_FREE(mMeshes);
      mMeshes    = (Mesh **)PX_ALLOC(sizeof(Mesh *)*mMeshCount);
      Mesh **dst = mMeshes;
      MyMeshVector::iterator i;
      for (i=mMyMeshes.begin(); i!=mMyMeshes.end(); ++i)
      {
    ...
    mMeshCollisionCount = (PxU32)mCollisionReps.size();

    if ( mMeshCollisionCount )
    {
      mMeshCollisionRepresentations = &mCollisionReps[0];
      for (PxU32 i=0; i<mMeshCollisionCount; i++)
      {
  ...
  {
    mMyMaterials.clear();
    PxU32 mcount = (PxU32)mMaterialMap.size();
  ...
  virtual void        importUserBinaryData(const char *name,PxU32 len,const PxU8 *data)
  {
    assert(0); // not yet implemented
  }

  virtual void        importTetraMesh(const char *tetraName,const char *meshName,PxU32 tcount,const PxF32 *tetraData)
  {
    assert(0); // not yet implemented
  }
  ...
  virtual void        importMesh(const char *meshName,const char *skeletonName)       // name of a mesh and the skeleton it refers to.
  {
    ...
    if ( mCurrentMesh == 0 )
    {
      MyMesh *m = PX_NEW(MyMesh)(m1.Get(),s1.Get());
  ...
  virtual void importPlane(const PxF32 *p)
  {
	  mPlane[0] = p[0];
	  mPlane[1] = p[1];
	  mPlane[2] = p[2];
  ...
  {
    MeshAnimation *a = MIB_NEW(MeshAnimation);
  ...
  virtual void        importRawTexture(const char *textureName,const PxU8 *pixels,PxU32 wid,PxU32 hit)
  {
//    assert(0); // not yet implemented
  }

  virtual void        importMeshInstance(const char *meshName,const PxF32 pos[3],const PxF32 rotation[4],const PxF32 scale[3])
  {
  ...
  virtual void importCollisionRepresentation(const char *name,const char *info) // the name of a new collision representation.
  {
    ...
    {
      MyMeshCollisionRepresentation *mcr = MIB_NEW(MyMeshCollisionRepresentation)(ref1.Get(),ref2.Get());
                            ...
                            PxF32 radius)
  {
    ...
    MeshCollisionSphere *c = MIB_NEW(MeshCollisionSphere);
    c->mName = mStrings.Get(boneName).Get();
    if ( transform )
    {
      memcpy(c->mTransform,transform,sizeof(PxF32)*16);
    }
    c->mRadius = radius;
  ...
  {
    getCurrentRep(collision_rep);
    MeshCollisionCapsule *c = MIB_NEW(MeshCollisionCapsule);
    c->mName = mStrings.Get(boneName).Get();
    if ( transform )
    {
      memcpy(c->mTransform,transform,sizeof(PxF32)*16);
  ...
  {
    getCurrentRep(collision_rep);
    MeshCollisionBox *c = MIB_NEW(MeshCollisionBox);
    c->mName = mStrings.Get(boneName).Get();
    if ( transform )
    {
      memcpy(c->mTransform,transform,sizeof(PxF32)*16);
  ...
  {
    getCurrentRep(collision_rep);
    MeshCollisionConvex *c = MIB_NEW(MeshCollisionConvex);
    c->mName = mStrings.Get(boneName).Get();
    if ( transform )
    {
      memcpy(c->mTransform,transform,sizeof(PxF32)*16);
    ...
    if ( c->mVertexCount )
    {
      c->mVertices = (PxF32 *)PX_ALLOC(sizeof(PxF32)*vertex_count*3);
      memcpy(c->mVertices,vertices,sizeof(PxF32)*vertex_count*3);
    }
    c->mTriCount = tri_count;
    if ( c->mTriCount )
    {
      c->mIndices = (PxU32 *)PX_ALLOC(sizeof(PxU32)*tri_count*3);
      memcpy(c->mIndices,indices,sizeof(PxU32)*tri_count*3);
    }
    MeshCollision *mc = static_cast< MeshCollision *>(c);
    mCurrentCollision->mGeometries.push_back(mc);
  }

  virtual void rotate(PxF32 rotX,PxF32 rotY,PxF32 rotZ)
  {
    PxF32 quat[4];
...

    {
      ...
      {
        MeshSkeleton *ms = (*i);
        for (PxI32 j=0; j<ms->mBoneCount; j++)
        {
...

    {
      ...
      {
        MyMesh *m = (*i);
        PxU32 vcount = m->mVertexPool.GetSize();
        if ( vcount > 0 )
        {
          MeshVertex *vb = m->mVertexPool.GetBuffer();
          for (PxU32 j=0; j<vcount; j++)
          {
...

    {
      ...
      {
        MeshAnimation *ma = (*i);
        for (PxI32 j=0; j<ma->mTrackCount && j <1; j++)
        {
          MeshAnimTrack *t = ma->mTracks[j];
          for (PxI32 k=0; k<t->mFrameCount; k++)
          {
  ...
  virtual void scale(PxF32 s)
  {
...

    {
      ...
      {
        MeshAnimation *ma = (*i);
        for (PxI32 j=0; j<ma->mTrackCount; j++)
        {
          MeshAnimTrack *t = ma->mTracks[j];
          for (PxI32 k=0; k<t->mFrameCount; k++)
          {
  ...
  virtual PxI32 getSerializeFrame(void)
  {
...
MeshBuilder * createMeshBuilder(KeyValueIni *ini,const char *meshName,const void *data,PxU32 dlen,MeshImporter *mi,const char *options,MeshImportApplicationResource *appResource)
{
  MyMeshBuilder *b = PX_NEW(MyMeshBuilder)(ini,meshName,data,dlen,mi,options,appResource);
  return static_cast< MeshBuilder *>(b);
}

MeshBuilder * createMeshBuilder(MeshImportApplicationResource *appResource)
{
  MyMeshBuilder *b = PX_NEW(MyMeshBuilder)(appResource);
  return static_cast< MeshBuilder *>(b);
}

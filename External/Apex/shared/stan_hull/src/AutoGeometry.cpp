	...
	public:
		...
		float generateHull(void)
		{
			...
			if ( mPoints.size() >= 3 ) // must have at least 3 vertices to create a hull.
			{
				...
				physx::stanhull::HullDesc desc((physx::stanhull::HullFlag)(physx::stanhull::QF_TRIANGLES | physx::stanhull::QF_SKIN_WIDTH), (physx::PxU32)mPoints.size(),&mPoints[0].x, sizeof(float)*3);
				...
				if ( e == physx::stanhull::QE_OK )
				{
					...
					mIndices  = (unsigned int *)PX_ALLOC(sizeof(unsigned int)*mTriCount*3);
					memcpy(mIndices,result.mIndices,sizeof(unsigned int)*mTriCount*3);
					mVertexCount = result.mNumOutputVertices;
					mVertices = (float *)PX_ALLOC(sizeof(float)*mVertexCount*3);
...
#define MAX_BONE_COUNT 8
			...
			unsigned int &geom_count)
		{
			...
			if ( geom_count )
			{
				mSimpleHulls = (physx::SimpleHull **)PX_ALLOC(sizeof(physx::SimpleHull *)*geom_count);
				int index = 0;
				for (int i=0; i<(int)bone_count; i++)
				{

...
bool PAUtils::lock()
{
    if( !mImpl || !mImpl->submitEventLock )
    {
        ...
        if( !reportOnce )
            shdfnd2::getFoundation().error(PX_INFO, "The AGPerfMON DLL does not contain a lock() method, GPU profiling disabled.");
        reportOnce = true;
        return false;
    }
...
#   define NTDLL_LIB_NAME               "NTDLL.DLL"
    ...
    if( !mPmDll )
    {
		//shdfnd2::getFoundation().error(PX_INFO, "Unable to find AgPerfMon.dll");
        return;
    }
    ...
    {
		shdfnd2::getFoundation().error(PX_INFO,

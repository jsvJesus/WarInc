...
namespace shdfnd2
{
	...
	#define BEGIN_FIELDS(current_class)		physx::shdfnd2::FieldDescriptor current_class::mClassDescriptor[] = {	\
	...
	#define DECLARE_FIELDS																			\
																									\
	static physx::shdfnd2::FieldDescriptor		mClassDescriptor[];									\
																									\
	...
	const physx::shdfnd2::FieldDescriptor* getDescriptor()	const	{ return mClassDescriptor;	}	\
	/* Finds a field given its name */																\
	const physx::shdfnd2::FieldDescriptor* findDescriptor(const char* name)	const					\
	{																								\
		if(!name)	return NULL;																	\
		const PxU32 Size = getDescriptorSize();														\
		for(PxU32 i=0;i<Size;i++)																	\
		{																							\
			const physx::shdfnd2::FieldDescriptor* FD = &mClassDescriptor[i];						\
	...
	virtual	PxU32		getObjectSize()	const	{ return sizeof(*this);		}						\
																									\
	virtual bool	getFields(physx::shdfnd2::FieldDescriptors& edit, PxU32 flags)	const			\
	{																								\
		base_class::getFields(edit, flags);															\
		const physx::shdfnd2::FieldDescriptor* Fields = current_class::getDescriptor();				\
		...
		return true;																				\
																									...
																									\
	virtual bool getFields(physx::shdfnd2::FieldDescriptors& edit, PxField::Enum type)	const		\
	{																								\
		...
		return true;																				\
																									...
																									\
	virtual bool getFields(physx::shdfnd2::FieldDescriptors& edit)	const							\
	{																								\
		...
		return true;																				\
																									...
																									\
	virtual const physx::shdfnd2::FieldDescriptor* getFieldDescriptor(const char* name)	const		\
	{																								\
		if(!name)	return NULL;																	\
		const physx::shdfnd2::FieldDescriptor* FD = base_class::getFieldDescriptor(name);			\
		if(FD)	return FD;																			\
																									\

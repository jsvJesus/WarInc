...
namespace shdfnd2
{
	...
	*/
	...
	public:
		...
		// PX_SERIALIZATION
		/* todo: explicit */ Array(PxRefResolver& v) : Alloc(v)
		{
			if(mData)
		...
		void				exportArray(PxSerialStream& stream, bool finalize)
		{
			...
			if(finalize)
				shrink();
			shdfnd2::exportArray(stream, mData, mSize, sizeof(T), capacity());
		}

		// PT: import must be as fast as possible, so inline here
		PX_FORCE_INLINE char*		importArray(char* address)
		{
			return shdfnd2::importArray(address, (void**)&mData, mSize, sizeof(T), capacity());
		}
		//~PX_SERIALIZATION
		...
		*/
		PX_INLINE explicit Array(const T* first, const T* last, const Alloc& alloc = Alloc())
			: Alloc(alloc), mSize(last<first?0:last-first), mCapacity(mSize), mData(allocate(mSize))
		{
...
//~PX_SERIALIZATION
		...
		*/
		...
		Note, does not call constructors or destructors, so use for simple types only.
		Operation is O(n)
		\param begin

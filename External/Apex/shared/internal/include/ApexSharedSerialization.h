...
PX_INLINE void serialize( physx::PxFileBuf& stream, const ConvexHull& h )
{
	if( h.mParams == NULL )
	{
		NX_ASSERT( !"Attempting to serialize a ConvexHull with NULL NxParameters." );
		return;
	}

	DeclareArray(physx::PxVec3)	vertices;
	DeclareArray(NxPlane)	uniquePlanes;
	physx::Array<physx::PxU32>			edges;
	physx::Array<physx::PxF32>			widths;

	NxParameterized::Handle handle(*h.mParams);

	// vertices
	vertices.resize( h.mParams->vertices.arraySizes[0] );
	h.mParams->getParameterHandle( "vertices", handle );
	h.mParams->getParamVec3Array( handle, vertices.begin(), vertices.size() );
	serialize( stream, vertices );

	// uniquePlanes
	uniquePlanes.resize( h.mParams->uniquePlanes.arraySizes[0] );
	for( int i = 0; i < h.mParams->uniquePlanes.arraySizes[0]; ++i )
	{
		ConvexHullParametersNS::Plane_Type& plane = h.mParams->uniquePlanes.buf[i];
		uniquePlanes[i].set( plane.normal.x, plane.normal.y, plane.normal.z, plane.d );
	}
	...
	// edges
	edges.resize( h.mParams->edges.arraySizes[0] );
	h.mParams->getParameterHandle( "edges", handle );
	h.mParams->getParamU32Array( handle, edges.begin(), edges.size() );
	serialize( stream, edges );

	// widths
	widths.resize( h.mParams->widths.arraySizes[0] );
	h.mParams->getParameterHandle( "widths", handle );
	h.mParams->getParamF32Array( handle, widths.begin(), widths.size() );
...
PX_INLINE void deserialize( physx::PxFileBuf& stream, physx::PxU32 version, ConvexHull& h )
{
	...
	h.mParams->getParameterHandle( "vertices", handle );
	h.mParams->resizeArray( handle, vertices.size() );
	h.mParams->setParamVec3Array( handle, vertices.begin(), vertices.size() );

	// uniquePlanes
	deserialize( stream, version, uniquePlanes );
	h.mParams->getParameterHandle( "uniquePlanes", handle );
	h.mParams->resizeArray( handle, uniquePlanes.size() );
	for( physx::PxU32 i = 0; i < uniquePlanes.size(); ++i )
	{
		NxPlane& plane = uniquePlanes[i];
		ConvexHullParametersNS::Plane_Type& paramPlane = h.mParams->uniquePlanes.buf[i];
		paramPlane.normal.set( plane.normal.x, plane.normal.y, plane.normal.z );
	...
	if( version >= ApexStreamVersion::ConvexHull_EdgesStreamed )
	{
		...
		h.mParams->getParameterHandle( "edges", handle );
		h.mParams->resizeArray( handle, edges.size() );
		h.mParams->setParamU32Array( handle, edges.begin(), edges.size() );
		...
		if( version >= ApexStreamVersion::ConvexHullModifiedForEfficientCollisionTests )
		{
			...
			h.mParams->getParameterHandle( "widths", handle );
			h.mParams->resizeArray( handle, widths.size() );
			h.mParams->setParamF32Array( handle, widths.begin(), widths.size() );
...
PX_INLINE void serialize( physx::PxFileBuf& stream, 
						  ...
						  NxParameterized::Handle &childHandle)
{
	PX_FORCE_PARAMETER_REFERENCE( headerVersion );
	PX_FORCE_PARAMETER_REFERENCE( param );
	PX_ASSERT( childHandle.getInterface() == param );
	...
	if( childHandle.parameterDefinition()->type() == NxParameterized::TYPE_REF )
	{
		...
		//if( param->initParamRef( childHandle ) != NxParameterized::ERROR_NONE )
		//{
		//	return;
		//}
							...
							NxParameterized::Handle &childHandle)
{
	...
	if( childHandle.parameterDefinition()->type() == NxParameterized::TYPE_REF )
	{
		...
		childHandle.initParamRef(tmpString.c_str() );
		childHandle.getParamRef(childParamPtr );

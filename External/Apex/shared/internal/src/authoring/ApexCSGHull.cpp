...
{
	return (int)((const physx::Hull::Edge*)a)->m_indexF1 - (int)((const physx::Hull::Edge*)b)->m_indexF1;
}


/* Hull */

void physx::Hull::intersect( const physx::Plane& plane, physx::Real distanceTol )
{
	if( isEmptySet() )
	{
		return;
	...
	if( vertexCount == 0 )
	{
		...
		if( NxMath::abs( den ) <= EPS )
		{	//	Lines are parallel to plane
			//	This is essentially a 2-d algorithm, edges->vertices, faces->edges
			bool negClassEmpty = true;
			bool zeroClassEmpty = true;
			bool posClassEmpty = true;
			NxI8* edgeClass = (NxI8*)NxAlloca( (edges.size()+2)*sizeof( NxI8 ) );	// Adding 2 for "edges at infinity"
			struct FaceEdge { physx::PxI32	e1, e2; };
			FaceEdge* faceEdges = (FaceEdge*)NxAlloca( sizeof( FaceEdge )*faces.size()+1 );// Adding 1 for possible new face
			physx::PxU32* faceIndices = (physx::PxU32*)NxAlloca( sizeof( physx::PxU32 )*(faces.size()+1) );	// Adding 1 for possible new face
			physx::PxU32* faceRemap = (physx::PxU32*)NxAlloca( sizeof( physx::PxU32 )*(faces.size()+1) );	// Adding 1 for possible new face
			...
			{	// The inequalities are set up so that when distanceTol = 0, class 0 is empty and classes -1 and 1 are disjoint
				Edge& edge = edges[i];
				PX_ASSERT( faceEdges[edge.m_indexF1].e2 == 0x80000000 );
				faceEdges[edge.m_indexF1].e2 = i;
				PX_ASSERT( faceEdges[edge.m_indexF2].e1 == 0x80000000 );
				faceEdges[edge.m_indexF2].e1 = i;
			...
			for( physx::PxU32 i = 0; i < faces.size(); ++i )
			{
				Plane& face = faces[i];
				FaceEdge& faceEdge = faceEdges[i];
				PX_ASSERT( faceEdge.e2 != 0x80000000 || faceEdge.e1 != 0x80000000 );
				if( faceEdge.e2 == 0x80000000 || faceEdge.e1 == 0x80000000 )
				{
			...
			bool addFace = false;
			FaceEdge newFaceEdge = { 0x80000000, 0x80000000 };
			const physx::PxU32 newFaceIndex = faces.size();

			for( physx::PxU32 i = faces.size(); i--; )
			{
				physx::PxI32 clippedEdgeIndex = edges.size();
				Plane& face = faces[i];
				FaceEdge& faceEdge = faceEdges[i];
				PX_ASSERT( faceEdge.e2 != 0x80000000 && faceEdge.e1 != 0x80000000 );
				...
				switch( edgeClass[NxMath::abs( faceEdge.e2 )] )
				{
				case 1:
					if( edgeClass[NxMath::abs( faceEdge.e1 )] == -1 )
					{	// Clip face
						PX_ASSERT( ((face.normal()^planeNormal)|edgeDir) > 0 );
						PX_ASSERT( newFaceEdge.e1 == 0x80000000 );
				...
				case 0:
					...
					{	// Keep this face, and use this edge on the new face
						...
						edges[faceEdge.e2].m_indexF2 = newFaceIndex;
						addFace = true;
					}
					else
					{	// Eliminate face
					...
					break;
				case -1:
					switch( edgeClass[NxMath::abs( faceEdge.e1 )] )
					{
					case 1:		// Clip face
						{
						PX_ASSERT( ((planeNormal^face.normal())|edgeDir) > 0 );
						PX_ASSERT( newFaceEdge.e2 == 0x80000000 );
					...
					case 0:		// Keep this face, and use this edge on the new face
						...
						edges[faceEdge.e1].m_indexF1 = newFaceIndex;
						addFace = true;
						break;
					}
				}
				...
				if( newEdgeIndex < edges.size() )
				{
					...
					if( orig1Index >= 0 )
					{
						const Edge& e1 = edges[orig1Index];
						const Pos& o1 = getV0( e1 );
						const Real d1 = plane.distance( o1 );
						if( orig2Index >= 0 )
						{
							const Edge& e2 = edges[orig2Index];
						...
						else
						{
							...
							PX_ASSERT( NxMath::abs( cosTheta ) > EPS );
			...
			NxU8* edgeMarks = (NxU8*)NxAlloca( sizeof( NxU8 )*edges.size() );
			memset( edgeMarks, 0, sizeof( NxU8 )*edges.size() );
			...
			physx::PxI32* vectorOffsets = (physx::PxI32*)NxAlloca( sizeof( physx::PxI32 )*vectors.size() );
	...
	bool zeroClassEmpty = true;
	bool posClassEmpty = true;
	NxI8* vertexClass = (NxI8*)NxAlloca( vertexCount*sizeof( NxI8 ) );
	...
	Edge* newEdges = (Edge*)NxAlloca( (newFaceIndex + 1)*sizeof( Edge ) );
	...
	for( physx::PxU32 i = edges.size(); i--; )
	{
		...
		case EdgeType::Line:
			...
			if( den > 0 )	// Make sure the ray points in the correct direction
			{
				...
				NX_Swap( edge.m_indexF1, edge.m_indexF2 );
			}
			createNewEdge = true;
			}
			break;
	...
	physx::PxI32* vectorOffsets = (physx::PxI32*)NxAlloca( sizeof( physx::PxI32 )*vectors.size() );
	physx::PxI32* faceOffsets = (physx::PxI32*)NxAlloca( sizeof( physx::PxI32 )*faces.size() );
	memset( vectorOffsets, 0, sizeof( physx::PxI32 )*vectors.size() );
	...
	physx::PxI32 faceCount = 0;
	for( physx::PxU32 i = 0; i < faces.size(); ++i )
	{
		const bool copy = faceOffsets[i] > 0;
		faceOffsets[i] = faceCount - (physx::PxI32)i;
		if( copy )
		{
	...
	physx::PxU32* vectorIndices = (physx::PxU32*)NxAlloca( sizeof( physx::PxU32 )*vectors.size() );
	physx::PxU32* vectorRemap = (physx::PxU32*)NxAlloca( sizeof( physx::PxU32 )*vectors.size() );
	for( physx::PxU32 i = 0; i < vectors.size(); ++i )
	{
		vectorIndices[i] = vectorRemap[i] = i;
	}
	physx::PxI32 firstDirIndex = -1;
	physx::PxI32 lastPosIndex = vectors.size();
	for(;;)
	{	// Correct this
		while( ++firstDirIndex < (physx::PxI32)vectors.size() )
		{
			if( vectors[firstDirIndex][3] < (Real)0.5 )	// Should be 0 or 1, but in case some f.p. inexactness creeps in...
			{
				break;
			}
		}
		...
		{
			if( vectors[lastPosIndex][3] >= (Real)0.5 )	// Should be 0 or 1, but in case some f.p. inexactness creeps in...
			{
				break;
			}
		}
		...
		NX_Swap( vectors[firstDirIndex], vectors[lastPosIndex] );
		NX_Swap( vectorIndices[firstDirIndex], vectorIndices[lastPosIndex] );
		vectorRemap[vectorIndices[firstDirIndex]] = firstDirIndex;
		vectorRemap[vectorIndices[lastPosIndex]] = lastPosIndex;
	}
	vertexCount = firstDirIndex;	// Correct vertex count
...
physx::Real physx::Hull::calculateVolume() const
{
	if( isEmptySet() )
	{
		return (Real)0;
	...
	Edge* edgeList = (Edge*)NxAlloca( edgeListSize*sizeof( Edge ) );
	for( physx::PxU32 i = 0; i < edgeCount; ++i )
	{
		edgeList[i] = edges[i];
		edgeList[i+edgeCount] = edges[i];
		NX_Swap( edgeList[i+edgeCount].m_indexF1, edgeList[i+edgeCount].m_indexF2 );
		NX_Swap( edgeList[i+edgeCount].m_indexV0, edgeList[i+edgeCount].m_indexV1 );
	}
	...
	physx::PxU32* vertex0Locations = (physx::PxU32*)NxAlloca( vertexCount*sizeof( physx::PxU32 ) );
...
void physx::Hull::serialize( physx::PxFileBuf& stream ) const
{
...
void physx::Hull::deserialize( physx::PxFileBuf& stream, physx::PxU32 version )
{
	setToAllSpace();
...
bool physx::Hull::testConsistency( physx::Real distanceTol, physx::Real angleTol ) const
{
	...
	for( physx::PxU32 i = 0; i < vertexCount; ++i )
	{
		...
		for( physx::PxU32 j = 0; j < edges.size(); ++j )
		{
			...
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING, "Hull::testConsistency: 0-length edge found.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
		}
		if( count < 3 )
		{
			NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING, "Hull::testConsistency: vertex connected to fewer than 3 edges.",__FILE__,__LINE__ );
			PX_ALWAYS_ASSERT();
			return false;
		}
	}
	...
	if( edges.size() == 0 )
	{
		if( faces.size() >= 3 )
		{
			NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: face count 3 or greater, but with no edges.",__FILE__,__LINE__ );
			PX_ALWAYS_ASSERT();
			return false;
		}
		if( vertexCount > 0 )
		{
			NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: hull has vertices but no edges.",__FILE__,__LINE__ );
			PX_ALWAYS_ASSERT();
			return false;
		}
		return true;
	...
	for( physx::PxU32 i = 0; i < faces.size(); ++i )
	{
		...
		for( physx::PxU32 j = 0; j < edges.size(); ++j )
		{
			...
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: edge connecting face to itself.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
		}
		if( count == 0 )
		{
			NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: face has no edges",__FILE__,__LINE__ );
			PX_ALWAYS_ASSERT();
			return false;
		}
		if( count == 1 )
	...
	for( physx::PxU32 i = 0; i < edges.size(); ++i )
	{
		...
		{	// Edge is a line or ray 
			...
			if( NxMath::abs( d ) > distanceTol )
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: edge line/ray origin not on face.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
			d = faces[edge.m_indexF1].normal() | getDir( edge );
			if( NxMath::abs( d ) > angleTol )
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: edge line/ray direction not perpendicular to face.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
		...
		{	// Edge is a line segment 
			...
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: edge has a line/ray origin but a real destination point.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
			d = faces[edge.m_indexF1].distance( getV0( edge ) );
			if( NxMath::abs( d ) > distanceTol )
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: edge vertex 0 not on face.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
			...
			if( NxMath::abs( d ) > distanceTol )
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: edge vertex 1 not on face.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
			d = faces[edge.m_indexF2].distance( getV0( edge ) );
	...
	if( vertexCount == 0 )
	{
		if( !halfInfiniteFaces )
		{
			...
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: hull has edges but no vertices, with no half-infinite faces.  Face count should equal edge count but does not.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
		}
		else
		{
			...
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: hull has edges but no vertices, with half-infinite faces.  Face count should be one more than edge count but is not.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
		}
		...
		for( physx::PxU32 i = 1; i < edges.size(); ++i )
		{
			...
			if( (e|e) > (Real)EPS*EPS )
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: prism edges not all facing the same direction.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
		}
	...
	else
	{
		...
		if( !halfInfiniteEdges )
		{
			if( c != 2 )
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: bounded hull with faces, vertices and edges does not obey Euler's formula.",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
		}
		else
		{
			if( c != 1 )
			{
				NxGetApexSDK()->getOutputStream()->reportError(PxErrorCode::eDEBUG_WARNING,  "Hull::testConsistency: unbounded hull with faces, vertices and edges does not obey Euler's formula (with a vertex at infinity).",__FILE__,__LINE__ );
				PX_ALWAYS_ASSERT();
				return false;
			}
		}

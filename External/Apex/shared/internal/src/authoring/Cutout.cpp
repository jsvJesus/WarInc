...
PX_INLINE physx::PxF32 perpendicularDistanceSquared( const physx::PxVec3& v0, const physx::PxVec3& v1, const physx::PxVec3& v2 )
{
	...
	return baseLen2 > NX_EPS_F32*dotXY( leg, leg ) ? square( crossZ( base, leg ) )/baseLen2 : 0.0f;
}
...
struct CutoutVert
{
	...
	void	set( physx::PxI32 _cutoutIndex, physx::PxI32 _vertIndex ) { cutoutIndex = _cutoutIndex; vertIndex = _vertIndex; }
};
...
public:
				Map2d() : mMem( NULL ) {}
				Map2d( physx::PxU32 width, physx::PxU32 height ) : mMem( NULL ) { create_internal( width, height, NULL ); }
				Map2d( physx::PxU32 width, physx::PxU32 height, T fillValue ) : mMem( NULL ) { create_internal( width, height, &fillValue ); }
				Map2d( const Map2d& map ) { *this = map; }
				~Map2d() { delete [] mMem; }
	...
	void		create( physx::PxU32 width, physx::PxU32 height ) { return create_internal( width, height, NULL ); }
	void		create( physx::PxU32 width, physx::PxU32 height, T fillValue ) { create_internal( width, height, &fillValue ); }

	void		clear( const T value )
				{
					T* mem = mMem;
					T* stop = mMem + mWidth*mHeight;
					while( mem < stop ) { *mem++ = value; }
				}

	void		setOrigin( physx::PxU32 x, physx::PxU32 y ) { mOriginX = x; mOriginY = y; }

	const T&	operator () ( physx::PxI32 x, physx::PxI32 y ) const	{ x += mOriginX; y += mOriginY;	return mMem[x+y*mWidth]; }
	T&			operator () ( physx::PxI32 x, physx::PxI32 y )		{ x += mOriginX; y += mOriginY;	return mMem[x+y*mWidth]; }
...
public:
			BitMap() : mMem( NULL ) {}
			BitMap( physx::PxU32 width, physx::PxU32 height ) : mMem( NULL ) { create_internal( width, height, NULL ); }
			BitMap( physx::PxU32 width, physx::PxU32 height, bool fillValue ) : mMem( NULL ) { create_internal( width, height, &fillValue ); }
			BitMap( const BitMap& map ) { *this = map; }
			~BitMap() { delete [] mMem; }
	...
	void	create( physx::PxU32 width, physx::PxU32 height ) { return create_internal( width, height, NULL ); }
	void	create( physx::PxU32 width, physx::PxU32 height, bool fillValue ) { create_internal( width, height, &fillValue ); }

	void	clear( bool value )	{ memset( mMem, value ? 0xFF : 0x00, mRowBytes*mHeight ); }

	void	setOrigin( physx::PxU32 x, physx::PxU32 y ) { mOriginX = x; mOriginY = y; }

	bool	read( physx::PxI32 x, physx::PxI32 y ) const		{ x += mOriginX;	y += mOriginY;	return ((mMem[(x>>3)+y*mRowBytes]>>(x&7))&1) != 0; }
	void	set( physx::PxI32 x, physx::PxI32 y )				{ x += mOriginX;	y += mOriginY;	mMem[(x>>3)+y*mRowBytes] |= 1<<(x&7); }
	void	reset( physx::PxI32 x, physx::PxI32 y )			{ x += mOriginX;	y += mOriginY;	mMem[(x>>3)+y*mRowBytes] &= ~(1<<(x&7)); }
...
PX_INLINE physx::PxF32 compareTraceSegmentToLineSegment( const physx::Array<POINT2D>& trace, int start, int delta, physx::PxF32 distThreshold, physx::PxU32 width, physx::PxU32 height )
{
	if( delta < 2 )
	{
		return 0.0f;
	...
	const bool startIsOnBorder = trace[start].x == -1 || trace[start].x == (int)width || trace[start].y == -1 || trace[start].y == (int)height;
	const bool endIsOnBorder = trace[end].x == -1 || trace[end].x == (int)width || trace[end].y == -1 || trace[end].y == (int)height;
	...
	for(;;)
	{
		...
		testDisp -= orig;
		aveError += (physx::PxF32)( NxMath::abs( testDisp.x*dir.y - testDisp.y*dir.x ) >= distThreshold );
	}
...
PX_INLINE physx::PxF32 segmentsIntersectXY( const physx::PxVec3& v0, const physx::PxVec3& e0, const physx::PxVec3& v1, const physx::PxVec3& e1 )
{
	...
	physx::PxF32 a1 = NxMath::abs( c10 );
	physx::PxF32 b1 = NxMath::abs( c10+d10 );
	...
	physx::PxF32 a2 = NxMath::abs( c01 );
	physx::PxF32 b2 = NxMath::abs( c01+d01 );
...
static void createCutout( physx::Cutout& cutout, const physx::Array<POINT2D>& trace, physx::PxF32 snapThreshold, physx::PxU32 width, physx::PxU32 height )
{
	...
	for( physx::PxU32 iStart = 0; iStart < size; ++iStart )
	{
		...
		for( ; iDelta > 1; --iDelta )
		{
			physx::PxF32 fit = compareTraceSegmentToLineSegment( trace, iStart, iDelta, CUTOUT_DISTANCE_THRESHOLD, width, height );
	...
	cutout.vertices.pushBack( physx::PxVec3( (physx::PxF32)trace[start].x, (physx::PxF32)trace[start].y, 0 ) );
	...
	while( (size -= delta) > 0 )
	{
		...
		{
			physx::PxF32 fit = compareTraceSegmentToLineSegment( trace, start, delta, CUTOUT_DISTANCE_THRESHOLD, width, height );
			if( fit < errorThreshold )
			{
				break;
			}
...
static void splitTJunctions( physx::CutoutSet& cutoutSet, physx::PxF32 threshold )
{
	...
	for( physx::PxU32 i = 0; i < cutoutSet.cutouts.size(); ++i )
	{
		...
		for( physx::PxU32 j = 0; j < cutoutSize; ++j )
		{
			...
			bounds[edgeCount].aabb.fatten( threshold );
	...
	boundsCalculateOverlaps( overlaps, physx::Bounds3XY, bounds );
...
static void mergeVertices( CutoutSet& cutoutSet, physx::PxF32 threshold, physx::PxU32 width, physx::PxU32 height )
{
	...
	// Find bounds overlaps
	overlaps.reset();
	boundsCalculateOverlaps( overlaps, Bounds3XY, bounds );
...
static void mergeVertices( physx::CutoutSet& cutoutSet, physx::PxF32 threshold, physx::PxU32 width, physx::PxU32 height )
{
	...
	for( physx::PxU32 i = 0; i < vertexCount; ++i )
	{
		...
		const bool isOnBorder0 = isOnBorder( vert0, width, height );
		for( physx::PxU32 j = start; j < stop; ++j )
		{
			...
			const bool isOnBorder1 = isOnBorder( vert1, width, height );
			if( isOnBorder0 != isOnBorder1 )
			{	// No border/non-border pairs
				continue;
			}
			if( vert0.distanceSquared( vert1 ) > threshold2 )
	...
	for( physx::PxU32 i = 0; i < vertexCount; ++i )
	{
		...
		while( groupStart < stop )
		{
			...
			physx::PxF32 minDist2 = vert0.distanceSquared( vert1 );
			while( ++next < stop )
			{
				...
				const physx::PxF32 dist2 = vert0.distanceSquared( vertNext );
				if( dist2 < minDist2 )
				{
...
static bool decomposeCutoutIntoConvexLoops( physx::Cutout& cutout, physx::PxF32 cleanupTolerance = 0.0f )
{
	...
	physx::PxVec3 prevV = (cutout.vertices[size-1] - center).arrayMultiply( scale );
	physx::PxF32 area = 0.0f;
	for( physx::PxU32 i = 0; i < size; ++i )
	{
		const physx::PxVec3 v = (cutout.vertices[i] - center).arrayMultiply( scale );
		area += crossZ( prevV, v );
		prevV = v;
	}
	...
	for( physx::PxU32 i = 0; i < size; ++i )
	{
		...
		vert.index = (NxU16)(ccw ? i : size-i-1);
		vert.flags = 0;
	}
			...
			for( physx::PxU32 k = 0; k < loopSize; ++k )
			{
				...
				for( physx::PxU32 m = k+2; m < mStop; ++m )
				{
					...
					else
					{
						...
						if( !cleanSliceFound && len2 < minLen2 )
						{
							minLen2 = len2;
							kToUse = k;
							mToUse = m;
...
//		bool canCorner = false;
		...
		{
//			const bool oldDirRect = ((dirN&1) == 0);
			--dirN;
			const POINT2D t1 = POINT2D( t.x + taxicabSine(dirN+2), t.y + taxicabSine(dirN) );
			if( regions( t1.x, t1.y ) != regionIndex )
			{
...
//				if( oldDirRect && i == 3 )	// Try not to cut corners
//				{
//					canCorner = true;
//					continue;
//				}
...
//			if( i == 4 && canCorner )
//			{
//				t.x += taxicabSine(dirN+3);
//				t.y += taxicabSine(dirN+1);
//				++pathCounts( t.x, t.y );	// Increment path count
//				dirN += 5;
//				break;
//			}
...
static void createCutoutSet( physx::CutoutSet& cutoutSet, const NxU8* pixelBuffer, physx::PxU32 bufferWidth, physx::PxU32 bufferHeight, physx::PxF32 snapThreshold )
{
	...
	BitMap map( bufferWidth+2, bufferHeight+2, 0 );	// Padded for borders
	map.setOrigin( 1, 1 );
	...
	// Create a region map
	Map2d<physx::PxU32> regions( bufferWidth+2, bufferHeight+2, 0xFFFFFFFF );	// Initially an invalid value
	regions.setOrigin( 1, 1 );

	// Create a path counting map
	Map2d<physx::PxU8> pathCounts( bufferWidth+2, bufferHeight+2, 0 );
	pathCounts.setOrigin( 1, 1 );
	...
	for( physx::PxI32 y = 0; y < (physx::PxI32)bufferHeight; ++y )
	{
		for( physx::PxI32 x = 0; x < (physx::PxI32)bufferWidth; ++x )
		{
			...
			{	// Found an empty spot
				...
				traces.back() = (physx::Array<POINT2D>*)PX_ALLOC( sizeof( physx::Array<POINT2D> ) );
	...
	const physx::PxU32 cutoutCount = traces.size();
	...
	for( physx::PxU32 i = 0; i < cutoutCount; ++i )
	{
		createCutout( cutoutSet.cutouts[i], *traces[i], snapThreshold, bufferWidth, bufferHeight );
	}
...
{
	// original version
	PX_FORCE_PARAMETER_REFERENCE(version);
	stream >> p.index >> p.flags;
}
...
namespace FractureTools
{
	...
	void buildCutoutSet( ICutoutSet& cutoutSet, const NxU8* pixelBuffer, physx::PxU32 bufferWidth, physx::PxU32 bufferHeight, physx::PxF32 snapThreshold )
	{
		::createCutoutSet( *(physx::CutoutSet*)&cutoutSet, pixelBuffer, bufferWidth, bufferHeight, snapThreshold );
	}
} // namespace FractureTools

...
#if defined(SF_OS_PS3) || defined(SF_OS_ANDROID)
#include <unistd.h>
#endif
...
public:
    ...
    FILEFile(FILE *pfile, const char* pfileName, int closeFd);
...
FILEFile::FILEFile(FILE *pfile, const char *pfileName, int closeFd)
{
    FileName = pfileName;
    Opened = 1;
    OpenFlags = Open_Read|Open_Write;
    if (closeFd)
        OpenFlags |= FILE_CloseFd;
    ErrorCode = 0;
    LastOp = 0;
    fs = pfile;
}
...
void FILEFile::init()
{
    ...
    // X360 only accepts backslashes as separator characters. Replace them in the filename.
    StringBuffer sb;
    for ( unsigned i = 0; i < FileName.GetLength(); ++i )
    {
        char c = FileName[i];
        if ( c != '/' )
            sb.AppendChar(c);
...
File *FileFILEOpen(FILE *file, const char* path, int closeFd)
{
    return SF_NEW FILEFile(file, path, closeFd);
}

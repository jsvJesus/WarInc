package warz.vault {
	...
	public class Inventory extends MovieClip {
		...
		public var api:warz.vault.vault=null;
		...
		public function Inventory() {
			...
			this.BtnChange.Text.Text.text = "$FR_PAUSE_INVENTORY_CHANGE_BACKPACK";
			this.BtnChange.visible = false;

			for(var i=1; i<=7; ++i)
			{
				var Name = "Tab"+i;
				this[Name].Btn.addEventListener(MouseEvent.MOUSE_OVER, ButtonRollOverFn);
				this[Name].Btn.addEventListener(MouseEvent.MOUSE_OUT, ButtonRollOutFn);
				this[Name].Btn.addEventListener(MouseEvent.CLICK, ButtonPressFn);
			}
			...
			this.Title1.text = "$FR_INVENTORY_GOLBAL_INVENTORY";
		...
		public function Activate()
		{
			...
			for(var i=1; i<=7; ++i)
			{
				...
				if (this[Name].currentLable != "out")
				{
					this[Name].State = "off";
					this[Name].gotoAndPlay("out");
				}
				
				if (this[Name].Icon.currentLabel != tab.name)
				{
			...
			if (this["Tab1"].currentLabel != "pressed")
			{
		...
		public function ActionFunction (button:String)
		{
			...
			else if (button == "BtnChange")
			{
				...
				//VaultEvents.eventOpenBackpackSelector ();
			}
		}
		...
		public	function fillInventory (tabID:int)
		{
			...
			for (i = 0; i < sortedItemDB.length; i++)
			{
				...
				{
					inventorySlot = new warz.vault.InvTableSlot();
					...
					if (DraggedItem == null)
					{
						...
						if (inventoryItem)
						{
							...
							if(isShiftKeyDown && inventoryItem.quantity>1)
							{
								...
								VaultEvents.eventBackpackFromInventory (inventoryItem.inventoryID, -1, 1);
						}
					}
...
// move item to this backpack slot					
					if (DraggedItem is warz.vault.InvBackpackHotSlot ||
						DraggedItem is warz.vault.InvBackpackSlot ||
						DraggedItem is warz.vault.InvMainSlotLong ||
						DraggedItem is warz.vault.InvMainSlotShort)
					{
						if (api.isDebug)
						{
							var survivor:Survivor = api.survivor;
						...
						else 
						{
							if(isShiftKeyDown && backpackItem.quantity>1)
							{
								...
								VaultEvents.eventBackpackToInventory (backpackItem.slotID, backpackItem.quantity);
						}
					...
					if (DraggedItem == null)
					{
						...
						if (backpackItem)
						{
							...
							if(isShiftKeyDown && backpackItem.quantity>1)
							{
								...
								VaultEvents.eventBackpackToInventory (backpackItem.slotID, 1);
						}					
					}
...
// move item to this backpack slot					
					if (isOK &&
						DraggedItem is warz.vault.InvTableSlot)
					{
						...
						if (isOK)
						{
							...
							else 
							{
								if(isShiftKeyDown && inventoryItem.quantity>1)
								{
									...
									VaultEvents.eventBackpackFromInventory (inventoryItem.inventoryID, slot, inventoryItem.quantity);
							}
						}
					}
					else if (isOK &&
							 (DraggedItem is warz.vault.InvBackpackHotSlot ||
							 DraggedItem is warz.vault.InvBackpackSlot ||
							 DraggedItem is warz.vault.InvMainSlotLong ||
							 DraggedItem is warz.vault.InvMainSlotShort))
					{
						...
						if (isOK)
						{
							...
							else
							{
								...
								{
									VaultEvents.eventBackpackGridSwap (backpackItem.slotID, slot);
								}
							}
		...
		public	function eventBackpackFromInventoryCallback (state:Boolean, text:String=""):void
		{
			if (state)
			{
				...
				if(q>0)
					VaultEvents.eventBackpackFromInventory (MoveItem_inventoryID, MoveItem_slot, Math.min(q, MoveItem_Quantity));
			}
		}
		...
		public	function eventBackpackToInventoryCallback (state:Boolean, text:String=""):void
		{
			if (state)
			{
				var q:uint = uint(text);
				if(q>0)
					VaultEvents.eventBackpackToInventory (MoveItem_slot, Math.min(MoveItem_Quantity, q));
			}
		}		
...
// backpack info
			...
			for (i = 0; i < survivor.getBackpackSize (); i++)
			{
				...
				if (i == 0)
				{
					if (!backpackSlot)
					{
						backpackSlot = new warz.vault.InvMainSlotLong();
						isNew = true;
					}
				...
				else if (i == 1 || i == 6 || i == 7)
				{
					if (!backpackSlot)
					{
						backpackSlot = new warz.vault.InvMainSlotShort();
						isNew = true;
					}
				...
				else if (i < 8)
				{
					if (!backpackSlot)
					{
						backpackSlot = new warz.vault.InvBackpackHotSlot();
						isNew = true;
					}
				...
				else 
				{
					if (!backpackSlot)
					{
						backpackSlot = new warz.vault.InvBackpackSlot();
						isNew = true;
					}

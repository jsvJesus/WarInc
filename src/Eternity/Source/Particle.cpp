...
static void psReloadTexture( r3dTexture*& tex )
{
	...
	tex->Unload();
	tex->Load( FileName, D3DFMT_FROM_FILE, GetParticleDownScale() );
}
...
void r3dParticleSystemInit()
{
	...
	partTrailCache = new r3dParticleTrailCache();
...
r3dParticleSystem* r3dParticleSystemLoad(const char* fname)
{
	...
	if(NumParticleCache >= PARTICLE_CACHE_SIZE) {
		...
		return new r3dParticleSystem(ParticleCache[0]);
	}
	...
	for(int i=0; i<NumParticleCache; i++) 
	{
		if(stricmp(ParticleCache[i]->FileName, fname) == NULL) {
			...
			if(_r3d_ParticleEditorActive) {
				// in editor, we need to reload particle data, to discard changes maded
				pd->Load(pd->FileName);
				r3dOutToLog("particle: reloaded '%s'\n", fname);
			}

			return new r3dParticleSystem(pd);
		}
	}
	...
	r3dParticleData* pd = new r3dParticleData();
	if(!pd->Load(fname)) {
		delete pd;
...
{
  if(MeshTexture) {
    r3dRenderer->DeleteTexture(MeshTexture);
    MeshTexture = NULL;
  }
  
  if(MeshDistortTexture) {
	  r3dRenderer->DeleteTexture(MeshDistortTexture);
	  MeshDistortTexture = NULL;
  }
...
void r3dParticleEmitter::Copy(const r3dParticleEmitter* PE, const char* NewName)
{
	...
	if(ParticleType == R3D_PARTICLE_MESH)
	{
		...
		MeshFName = PE->MeshFName;
		sprintf(TStr, "Data\\Particles\\%s", MeshFName.c_str());

		Mesh = r3dParticleSystemAddMesh(TStr);

		// main texture
		MeshTexFName = PE->MeshTexFName;
		sprintf(TStr, "Data\\Particles\\%s", MeshTexFName.c_str());

		MeshTexture = r3dRenderer->LoadTexture(TStr, D3DFMT_UNKNOWN, false, GetParticleDownScale() );
		MeshTexture->bPersistent = 1;

		bMeshUseDistortTexture = PE->bMeshUseDistortTexture;

		if ( bMeshUseDistortTexture )
		{
			// Distortion texture
			MeshDistortTexFName = PE->MeshDistortTexFName;
			sprintf(TStr, "Data\\Particles\\%s", MeshDistortTexFName.c_str());

			MeshDistortTexture = r3dRenderer->LoadTexture(TStr, D3DFMT_UNKNOWN, false, GetParticleDownScale());
			MeshDistortTexture->bPersistent = 1;
		}
	}
}
...
void r3dParticleEmitter::Load(const char* FName, const char* PartName)
{
	...
	if(ParticleType == R3D_PARTICLE_MESH)
	{
		...
		// main texture
		MeshTexFName = r3dReadCFG_S(FName, PartName, "MeshTexture", "arrow.bmp");
		sprintf(TStr, "Data\\Particles\\%s", MeshTexFName.c_str());
		...
		{
			// Distortion texture
			if( r3dCFGHas(FName, PartName, "DistortionTexture" ) )
			{
				MeshDistortTexFName = r3dReadCFG_S(FName, PartName, "DistortionTexture", "arrow.bmp");
			}
			else
			{
				MeshDistortTexFName = r3dReadCFG_S(FName, PartName, "MeshDistortTexture", "arrow.bmp");
			}
...
void r3dParticleEmitter::Save(const char* FName)
{
	...
	if(ParticleType == R3D_PARTICLE_MESH)
	{
		...
		r3dWriteCFG_S(FName, Name, "MeshTexture", MeshTexFName.c_str());
		r3dWriteCFG_I(FName, Name, "MeshUseDistortTexture", bMeshUseDistortTexture);
		r3dWriteCFG_S(FName, Name, "MeshDistortTexture", MeshDistortTexFName.c_str());
	}
...
void r3dParticleData::ReloadTextures()
{
	...
	for ( int i = 0; i < MAX_EMITTER_SLOTS; i++ )
	{
		if ( r3dParticleEmitter* el = PType[i] )
		{
			psReloadTexture( el->MeshTexture );
			psReloadTexture( el->MeshDistortTexture );
		}
	}
}
...
int r3dParticleData::Load(const char* FName)
{
	...
	{ 
		...
		if(!strstr(Str2,"-NONE-"))
		{
			PType[i] = new r3dParticleEmitter;
...
void r3dParticleSystem::CreateBuffers()
{
	...
	SAFE_DELETE_ARRAY(Array);
	Array = new r3dSingleParticle[ArraySize];
...
void r3dParticleSystem::Update(float CurTime, bool bUpdate)
{
	...
	*/  
	...
	if(r_particles_quality->GetInt() == r_particles_quality->GetMaxVal())	curLod = 1.0f;
...
R3D_FORCEINLINE float GetSunK( const r3dPoint3D& pos, const r3dCamera& Cam )
{
	...
	r3dVector v1 = (pos - Cam);         v1.y = 0; v1.Normalize();
...
void r3dParticleSystem::FillBuffersNew( const r3dCamera& cam, int bOnlyShadowCasters )
{
	...
	//turn off particles LOD for highest quality settings
	if(r_particles_quality->GetInt() == r_particles_quality->GetMaxVal())	curLod = 1.0f;
	if(r_particles_quality->GetInt() == r_particles_quality->GetMinVal())	curLod = R3D_MAX( 2.0f, curLod ) ;
	...
	for(int i=NumAliveParticles-1;i>=0;i--)
	{
...

		{
			...
			if(PE.Mesh)
			{
				...
				r3dRenderer->SetTex(PE.MeshTexture, 0);

				if(PE.bMeshUseDistortTexture && PE.MeshDistortTexture)
					r3dRenderer->SetTex(PE.MeshDistortTexture, 2);
				else
					r3dRenderer->SetTex(g_BlankDistortTex, 2);


				PE.Mesh->SetVSConsts(pt.Position, (1.0f/fScale)*GlobalRenderScale*r3dVector(pot.Size,pot.Size,pot.Size), mr);
...
void r3dParticleSystem::Draw( const r3dCamera &cam, bool bShadowMap )
{
	...
	if( !PD->Texture->IsLoaded() || 
		( PD->DistortTexture && !PD->DistortTexture->IsLoaded() ) ||
		( PD->NormalTexture && !PD->NormalTexture->IsLoaded() )
		)
	{
		return ;
	}
	...
	D3DXVECTOR4 vConsts[] = 
	{
		...
		D3DXVECTOR4( r3dGameLevel::Environment.ParticleShadingCoef, 0.f, g_ShadowMap->Width, g_ShadowMap->Height ),
	...
	if( !bShadowMap )
	{
		...
		if( PD->bReceivesShadows && r_shadows->GetInt() )
		{
	...
	if( r_instanced_particles->GetInt() )
	{
		...
		if( PD->bReceivesShadows && r_shadows->GetInt() && r_particle_shadows->GetInt() )
		{
	...
	for(int i=NumAliveParticles-1;i>=0;i--)
	{
...

		{
			...
			if(PE.Mesh)
			{
				...
				r3dRenderer->SetTex( __r3dShadeTexture[1],5);

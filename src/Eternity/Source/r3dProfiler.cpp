...
bool r3dProfiler::Create(int aNumThreads)
{
	...
	sInstance = new r3dProfiler[aNumThreads];

	g_pQueryPool = new QueryPool;
...
void r3dProfiler::Start(const char* aName, uint32_t aHashName)
{
	...
	if(sample==NULL)
	{
		...
		if(item == NULL)
		{
			item = new r3dProfilerItem(aName, aHashName);
			m_FirstProfileItem->AddLast(item);
		}
		sample = new r3dProfilerSample(item);
		//Add it to the tree
...
{
	m_CurrentFrame = 0;
	m_FirstProfileItem = new r3dProfilerItem("Start", r3dHash::MakeHash("Start"));
	m_RootSample = new r3dProfilerSample(m_FirstProfileItem);

	m_QueryItem = new r3dProfilerItem( "Occ. Queries", r3dHash::MakeHash( "Occ. Queries" ) );
	m_QuerySample = new r3dProfilerSample( m_QueryItem );

...
D3DVERTEXELEMENT9 r3dInstancedUnionMeshVertex::VBDecl[] = 
{
	...
	{0, 48, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD, 2}, // from texcoord 3 to 6 instanced data in r3dObj.cpp for mesh instancing!
...
int r3dMesh::GetMaterialsSize() const
{
	...
	{ 
		...
		if ( pMat )
		{
			...
			nSize += GetTexMemorySize( pMat->BumpTexture );
			nSize += GetTexMemorySize( pMat->GlossTexture );
			nSize += GetTexMemorySize( pMat->imgEnvPower );
			nSize += GetTexMemorySize( pMat->IBLTexture );
			nSize += GetTexMemorySize( pMat->DetailNormalTexture );
		}	
	}
	return nSize;
}
...
#ifndef FINAL_BUILD
	densityVisualizer = new MeshTextureDensityVisualizer(this);
#endif
}
...
void r3dMesh::RecalcBasisVectors()
{
	...
	struct Binormals
	{
		Binormals( int Count )
		{
			binormals = new r3dPoint3D[ Count ];
		}
...
namespace
{
	std::vector<int> tempIndexBuffer0 ;
	std::vector<int> tempIndexBuffer1 ;
...
template void r3dMesh::RemapComponent( r3dPoint3D* comp, const std::vector<int>& compMap ) ;
template void r3dMesh::RemapComponent( r3dPoint2D* comp, const std::vector<int>& compMap ) ;
template void r3dMesh::RemapComponent( char* comp, const std::vector<int>& compMap ) ;
template void r3dMesh::RemapComponent( r3dWeight* comp, const std::vector<int>& compMap ) ;
...
r3dMesh::FillMeshBuffers( r3dTaskParams* params )
{
	...
	{
		static_cast<r3dMesh*>( static_cast< r3dTaskPtrParams* >( params )->Ptr )->DoFillBuffers() ;
	}
...
void r3dMesh::DoFillBuffersMainThread()
{
	...
	if(pWeights) FillBuffersUnique<r3dSkinnedMeshVertex>();
	else
	{
		if( VertexFlags & vfPrecise )
		{
...
// 	vb_lock  = 0;
	...
	{
		UnsharedVertexBuffer = new r3dVertexBuffer( NumVertices, sizeof( T ) ) ;
		UnsharedIndexBuffer = new r3dIndexBuffer( NumIndices, false, sizeof( IndexType ) ) ;
...
void r3dPrepareMeshVSConsts(PrecalculatedMeshVSConsts &out, const D3DXMATRIX &world, const r3dPoint3D* ScaleBox, const r3dPoint2D* TexcUnpackScale )
{
	...
	memcpy( out.normalWorldView, &ShaderMat, sizeof out.normalWorldView ) ;

	if( TexcUnpackScale )
	{
		out.texcUnpackScale = float4( TexcUnpackScale->x, TexcUnpackScale->y, 0.f, 0.f ) ;
	}
	else
	{
		out.texcUnpackScale = float4( 1.f, 1.f, 1.f, 1.f ) ;
	}
}
...
//-----------------------------------------------------------------------
void r3dMesh::SetVSConsts(const r3dVector& pos, const r3dVector& scale, const D3DXMATRIX& rotation)
{
	D3DXMATRIX mWorld;
	mesh_CreateWorldMatrix(mWorld, vPivot, pos, scale, rotation);

	r3dMeshSetVSConsts( mWorld, &unpackScale, &texcUnpackScale );
}


void r3dMesh::SetVSConsts(const D3DXMATRIX& world)
{
...
void r3dMesh::DrawMeshInstanced(const D3DXMATRIX* world, int ShadowMap, bool force_render )
{
	...
	static const int maxInstancedInVB = MaxInstancesToDraw*100;
	if(!pInstancesVB)
	{
		...
		pInstancesVB = new r3dVertexBuffer( maxInstancedInVB, sizeof ( InstancedData ), 0, true );
	...
	if(numInstancesInArray == MaxInstancesToDraw || (force_render && numInstancesInArray>0))
	{
		...
		{
			SetVSConsts(InstanceArray[0].world);
			if( ShadowMap )
			{
				DrawMeshShadows();
			}
...
void
r3dMesh::DrawMeshStart( const r3dColor* objectColor )
{
...
void
r3dMesh::DrawMeshDeferredBatch( int batchIdx, UINT MatFlags )
{
	...
	batch.Mat->Start( pWeights ? R3D_MATVS_SKIN : R3D_MATVS_DEFAULT, MatFlags );
...
void r3dMesh::DrawMeshShadowsBatch( int batchIdx )
{
	...
	if (batch.Mat->Texture)
		r3dRenderer->SetTex( batch.Mat->Texture );
	else
		r3dRenderer->SetTex( __r3dShadeTexture[2]);
...
r3dMesh::FlushLoadingBuffers()
{
	...
	std::vector<int>().swap( tempIndexBuffer0 ) ;
	std::vector<int>().swap( tempIndexBuffer1 ) ;
	std::vector<int>().swap( remapIndices ) ;
...
void r3dMesh::AllocateWeights()
{
	r3d_assert( !(VertexFlags & vfPrecise) );
	r3d_assert(pWeights == NULL);
	pWeights     = new r3dWeight[NumVertices];
}

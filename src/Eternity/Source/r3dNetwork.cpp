...
void r3dNetwork::Update()
{
  ...
  while((p = impl->peer->Receive()) != NULL)
  {
	...
	// packet above FIRST_FREE_PACKET_ID is our data packets
	DWORD peerId = p->systemAddress.systemIndex;
	r3d_assert(peerId < impl->maxPeers);
...
static int RakNet_FillBindAddresses(RakNet::SocketDescriptor* binds, int port)
{
...
int r3dNetwork::CreateHost(int port, int numPeers)
{
  ...
  int numBinds = RakNet_FillBindAddresses(&sdescs[0], port);
...
void r3dNetwork::DisconnectPeer(DWORD peerId)
{
  r3d_assert(impl && impl->peer);
  r3d_assert(peerId < impl->maxPeers);
  
  //r3dOutToLog("r3dNetwork: disconnecting peer%d\n", peerId);
  impl->peer->CloseConnection(impl->peerData[peerId], true);
}

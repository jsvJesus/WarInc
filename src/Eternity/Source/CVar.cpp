...
{
	for (size_t i = 0, iEnd = vList.size(); i < iEnd; ++i)
	{
		if (stricmp(vList[i]->pName, name) == 0)
			return vList[i];
	}
...
int CVarContainer::Write(const char* fname, const char* prefix)
{
	...
	for (size_t i = 0, iEnd = vList.size(); i < iEnd; ++i)
	{
		if (vList[i]->pName[0] == '@')
		{
			if (prefix && strnicmp(vList[i]->pName + 1, prefix, plen) != 0)
				continue;
		...
		if (prefix && strnicmp(vList[i]->pName, prefix, plen) != 0)
			continue;

		fprintf(f, "%-23s = %-10s", vList[i]->pName, vList[i]->Put(buf));

		if (*vList[i]->pComment)
			fprintf(f, "; %s", vList[i]->pComment);
...
int cvars_Write(const char* fname, const char* prefix)
{
	return CVarContainer::This()->Write(fname, prefix);
}

int cvars_Read(const char* fname)
{
	return CVarContainer::This()->Read(fname);
}

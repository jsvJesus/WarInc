...
r3dFS_FileEntry* r3dFS_FileList::AddNew(const char* fname)
{
  ...
  r3dFS_FileEntry* fe = new r3dFS_FileEntry();
...
bool r3dFileSystem::OpenVolumesEx(bool forRead, bool fail_if_error)
{
  ...
  for(size_t i=0; i<MAX_VOLUMES; i++) 
  {
    ...
    HANDLE h = ::CreateFile(fname, forRead ? GENERIC_READ : GENERIC_ALL, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
bool r3dFileSystem::BuildNewArchive(const char* baseName)
{
  ...
  for(int i=0; i<numFiles; i++)
  {
    ...
    r3dOutToLog("[%04d/%04d] ratio:%.2fx, size:%.0f mb              \r", i, numFiles, cperc, (float)stat_csize / 1024 / 1024);

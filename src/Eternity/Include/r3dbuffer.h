...
static r3dVBufferT*	CreateClass(int size) {
	  r3dVBufferT* b = new r3dVBufferT(size);
	  return b;
	}
...
public:
  ...
  public:
	...
	{
		return new r3dScreenBuffer(p);
	}

	static r3dScreenBuffer* CreateEmptyClass(const char* name) {
		return new r3dScreenBuffer(name);
	}

...
SPInt     SF_STDCALL GetLength(const char* buf, SPInt buflen)
{
    const char* p = buf;
    SPInt length = 0;
...
UInt32  SF_STDCALL GetCharAt(SPInt index, const char* putf8str, SPInt length)
{
...
SPInt  SF_STDCALL GetByteIndex(SPInt index, const char *putf8str, SPInt length)
{
    const char* buf = putf8str;
...
int     SF_STDCALL GetEncodeCharSize(UInt32 ucs_character)
{
...
UPInt   SF_STDCALL DecodeString(wchar_t *pbuff, const char* putf8str, SPInt bytesLen)
{
...
bool    check_equal(const char* utf8_in, const UInt32* ucs_in)
{
    for (;;)
    {
        ...
        if (next_ucs == 0)
        {
            SF_ASSERT(next_ucs_from_utf8 == 0);
            break;
        }
    }
...
{
    {
        ...
        SF_ASSERT(check_equal(test8, test32));
    }
                ...
                if (c == '\n' || byte_counter >= LINE_SIZE - 2)
                {
                    ...
                    for (;;)
                    {
                        ...
                        *q++ = uc;
                        ...
                        SF_ASSERT(q < line_buffer_ucs + LINE_SIZE);
                        SF_ASSERT(p < line_buffer_utf8 + LINE_SIZE);
                    ...
                    {
                        UInt32  uc = *q++;
                        SF_ASSERT(index < LINE_SIZE * 6 - 6);
                        int last_index = index;
                        UTF8Util::EncodeChar(reencoded_utf8, &index, uc);
                        SF_ASSERT(index <= last_index + 6);
                        if (uc == 0) break;
                    }
                    ...
                    SF_ASSERT(check_equal(line_buffer_utf8, line_buffer_ucs));
                    SF_ASSERT(check_equal(reencoded_utf8, line_buffer_ucs));

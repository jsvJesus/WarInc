...
#ifdef GFC_OS_MAC
        strcat(FileName, "/Contents/3DWindow.swf");
#else
        strcat(FileName, "/Bin/SWFToTexture/3DWindow.swf");
#endif
    }
...
//        GPtr<GFxFontProviderMac> fontProvider = *new GFxFontProviderMac(::GetDC(0));
  //      Loader.SetFontProvider(fontProvider);
    ...
    if (DoRender)
    {
        ...
        {
            printf("EXT_framebuffer_object is required.");
            return 0;
        }
...
bool    FxPlayerApp::LoadMovie(const char *pfilename)
{
    ...
    */
    ...
    if (sizeof(BGFileName) - 42 > strlen(apppath))
    {
        strcpy(BGFileName, apppath);
#ifdef GFC_OS_MAC
        strcat(BGFileName, "/Contents/3DWindowBackground.swf");
#else
        strcat(BGFileName, "/Bin/SWFToTexture/3DWindowBackground.swf");
#endif
    }

    pBGDef = *Loader.CreateMovie(BGFileName, loadConstants);
                                   ...
                                   int x, int y, unsigned int mods)
{
    ...
    MouseX = x = SInt(fx * RTWidth*0.5f + RTWidth*0.5f);
    MouseY = y = SInt(fy * RTHeight*0.5f + RTHeight*0.5f);
...
{
    bool ControlKeyDown = (mods & ControlMask);

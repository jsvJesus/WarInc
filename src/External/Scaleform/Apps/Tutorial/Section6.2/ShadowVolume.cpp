...
//	Ptr<RenderStats>  pRenderStats;
	...
	MovieDisplayHandle        hHUDMovieDisplay;
...
public:
						  ...
						  unsigned argCount)
	{
		...
		else if(strcmp(methodName, "NewBackground") == 0 && argCount == 1 && args[0].GetType() == Value::VT_Int)
		{
			g_nCurrBackground = (int)args[0].GetInt();
			GFxPrintf("g_nCurrBackground set to %d\n", g_nCurrBackground);
		}
		else if(strcmp(methodName, "NewLight") == 0 && argCount == 1 && args[0].GetType() == Value::VT_Int)
		{
			g_nNumLights = (int)args[0].GetInt() + 1;
			GFxPrintf("g_nNumLights set to %d\n", g_nNumLights);
		}
		else if(strcmp(methodName, "NewScene") == 0 && argCount == 1 && args[0].GetType() == Value::VT_Int)
		{
			int index = (int)args[0].GetInt();
			switch(index)
			{
...
bool GFxTutorial::InitGFx()
{
    ...
    Value listData[6];
    listData[0].SetString("Scene with shadow");
    listData[1].SetString("Show shadow volume");
    listData[2].SetString("Shadow volume complexity");
    pUIMovie->SetVariableArraySize("_root.SceneData", 3);
    pUIMovie->SetVariableArray(Movie::SA_Value, "_root.SceneData", 0, listData, 3);

    listData[0].SetString("Background : Cell");
    listData[1].SetString("Background : Field");
    pUIMovie->SetVariableArraySize("_root.BackgroundData", 2);
    pUIMovie->SetVariableArray(Movie::SA_Value, "_root.BackgroundData", 0, listData, 2);

    listData[0].SetString("1 light");
    listData[1].SetString("2 lights");
    listData[2].SetString("3 lights");
    listData[3].SetString("4 lights");
    listData[4].SetString("5 lights");
    listData[5].SetString("6 lights");
    pUIMovie->SetVariableArraySize("_root.LightsData", 6);
    pUIMovie->SetVariableArray(Movie::SA_Value, "_root.LightsData", 0, listData, 6);
	...
	hHUDMovieDisplay = pHUDMovie->GetDisplayHandle();
...
void GFxTutorial::OnResetDevice(IDirect3DDevice9* pd3dDevice)
{
    ...
    if(pd3dDevice->GetViewport(&vp) == D3D_OK)
    {
        ...
        if(pHUDMovie)
            pHUDMovie->SetViewport(vp.Width, vp.Height, 0, 0, vp.Width, vp.Height);
    }
}
...
void GFxTutorial::AdvanceAndRender(void)
{
	...
	if (hHUDMovieDisplay.NextCapture(pRenderer->GetContextNotify())) 
	{
		pRenderer->Display(hHUDMovieDisplay);
	}
...
void GFxTutorial::ProcessEvent(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool *pbNoFurtherProcessing)
{
	...
	if(pUIMovie)
	{
		...
		if(processedMouseEvent && pUIMovie->HitTest((float)mx, (float)my, Movie::HitTest_Shapes))
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    // Load the meshes
    V_RETURN( g_Background[0].Create( pd3dDevice, L"..\\Common\\misc\\cell.x" ) );

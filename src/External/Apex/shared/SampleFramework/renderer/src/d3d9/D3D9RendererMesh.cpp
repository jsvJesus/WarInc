...
static physx::PxU32 computePrimitiveCount(RendererMesh::Primitive primitive, physx::PxU32 vertexCount)
{
	physx::PxU32 numPrimitives = 0;
	switch(primitive)
	{
...
void D3D9RendererMesh::renderIndices(physx::PxU32 numVertices, physx::PxU32 firstIndex, physx::PxU32 numIndices, RendererIndexBuffer::Format indexFormat) const
{
...
void D3D9RendererMesh::renderVertices(physx::PxU32 numVertices) const
{
	...
	if(d3dDevice && m_d3dVertexDecl)
	{
		...
		physx::PxU32            numPrimitives = computePrimitiveCount(primitive, numVertices);
...
void D3D9RendererMesh::renderIndicesInstanced(physx::PxU32 numVertices, physx::PxU32 firstIndex, physx::PxU32 numIndices, RendererIndexBuffer::Format indexFormat,RendererMaterial *material) const
{
	...
	if(d3dDevice && m_d3dVertexDecl)
	{
		...
		d3dDevice->DrawIndexedPrimitive(getD3DPrimitive(primitive), 0, 0, numVertices, firstIndex, computePrimitiveCount(primitive, numIndices));
#else
		const physx::PxU8 *ibuffer = (const physx::PxU8 *)m_instanceBuffer->lock();
		ibuffer+=m_instanceBuffer->getStride()*m_firstInstance;
		D3DXMATRIX m;
		D3DXMatrixIdentity(&m);
		for (physx::PxU32 i=0; i<m_numInstances; i++)
		{
			physx::PxF32 *dest = (physx::PxF32 *)&m;

			const physx::PxF32 *src = (const physx::PxF32 *)ibuffer;
...
void D3D9RendererMesh::renderVerticesInstanced(physx::PxU32 numVertices,RendererMaterial *material) const
{

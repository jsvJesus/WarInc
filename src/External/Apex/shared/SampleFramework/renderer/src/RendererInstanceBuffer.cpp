...
physx::PxU32 RendererInstanceBuffer::getFormatByteSize(Format format)
{
	physx::PxU32 size = 0;
	switch(format)
	{
...
void *RendererInstanceBuffer::lockSemantic(Semantic semantic, physx::PxU32 &stride)
{
	...
	if(semantic < NUM_SEMANTICS)
	{
		...
		RENDERER_ASSERT(sm.format < NUM_FORMATS, "InstanceBuffer does not use this semantic.");

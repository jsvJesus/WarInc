	...
	RendererTexture2D(desc)
{
	...
	if(m_textureid)
	{
		...
		m_data = new physx::PxU8*[m_numLevels];
		memset(m_data, 0, sizeof(physx::PxU8*)*m_numLevels);
		...
		{
			for(physx::PxU32 i=0; i<desc.numLevels; i++)
			{
				physx::PxU32 w = getLevelDimension(m_width,  i);
				physx::PxU32 h = getLevelDimension(m_height, i);
				physx::PxU32 levelSize = computeImageByteSize(w, h, desc.format);
				m_data[i]       = new physx::PxU8[levelSize];
...
OGLRendererTexture2D::~OGLRendererTexture2D(void)
{
	...
	if(m_data)
	{
		for(physx::PxU32 i=0; i<m_numLevels; i++)
		{
			delete [] m_data[i];
		}
		delete [] m_data;
...
void *OGLRendererTexture2D::lockLevel(physx::PxU32 level, physx::PxU32 &pitch)
{
	void *buffer = 0;
...
void OGLRendererTexture2D::unlockLevel(physx::PxU32 level)
{
	RENDERER_ASSERT(level < m_numLevels, "Level out of range!");
	if(m_textureid && level < m_numLevels)
	{
		physx::PxU32 w = getLevelDimension(getWidth(),  level);
		physx::PxU32 h = getLevelDimension(getHeight(), level);
		glBindTexture(GL_TEXTURE_2D, m_textureid);
		if(isCompressedFormat(getFormat()))
		{
			physx::PxU32 levelSize = computeImageByteSize(w, h, getFormat());
...
void OGLRendererTexture2D::bind(physx::PxU32 textureUnit)
{
	if(m_textureid)
	{

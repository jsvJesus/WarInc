...
void OGLRendererMesh::renderIndices(physx::PxU32 numVertices, physx::PxU32 firstIndex, physx::PxU32 numIndices, RendererIndexBuffer::Format indexFormat) const
{
	const physx::PxU32 indexSize = RendererIndexBuffer::getFormatByteSize(indexFormat);
	void *buffer = ((physx::PxU8*)0)+indexSize*firstIndex;
	glDrawElements(getGLPrimitive(getPrimitives()), numIndices, getGLIndexType(indexFormat), buffer);
}

void OGLRendererMesh::renderVertices(physx::PxU32 numVertices) const
{
	RendererMesh::Primitive primitive = getPrimitives();
	glDrawArrays(getGLPrimitive(primitive), 0, numVertices);
}

void OGLRendererMesh::renderIndicesInstanced(physx::PxU32 numVertices, physx::PxU32 firstIndex, physx::PxU32 numIndices, RendererIndexBuffer::Format indexFormat,RendererMaterial *material) const
{
	const OGLRendererInstanceBuffer *ib = static_cast<const OGLRendererInstanceBuffer*>(getInstanceBuffer());
	if(ib)
	{
		const physx::PxU32 firstInstance = m_firstInstance;
		const physx::PxU32 numInstances  = getNumInstances();
		...
		{
			for(physx::PxU32 i=0; i<numInstances; i++)
			{
			#if defined(RENDERER_ENABLE_CG)
				const physx::PxMat34Legacy &model = ib->getModelMatrix(firstInstance+i);
				GLfloat glmodel[16];
				PxToGL(glmodel, model);
				
				physx::PxMat34Legacy modelView;
				modelView.multiply(m_renderer.getViewMatrix(), model);
				...
				renderIndices(numVertices, firstIndex, numIndices, indexFormat);
			}
		}
	}
}

void OGLRendererMesh::renderVerticesInstanced(physx::PxU32 numVertices,RendererMaterial *material) const
{
	const OGLRendererInstanceBuffer *ib = static_cast<const OGLRendererInstanceBuffer*>(getInstanceBuffer());
	if(ib)
	{

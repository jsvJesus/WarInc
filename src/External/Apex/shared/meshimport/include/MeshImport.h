...
{
public:
  virtual bool          sendMessage(PxU32 client,const char *fmt,...) = 0; // send a message to the server, all clients (client=0) or just a specific client.
  virtual const char *  receiveMessage(PxU32 &client) = 0; // receive an incoming message (client=0) means it came from the server, otherwise it designates a specific client.
  virtual const char ** getArgs(const char *input,PxU32 &argc) = 0; // parse string into a series of arguments.

  virtual bool          sendBlob(PxU32 client,const char *blobType,const void *data,PxU32 dlen) = 0;
  virtual const char *  receiveBlob(PxU32 &client,const void *&data,PxU32 &dlen) = 0;
...
inline PxF32 fmi_computePlane(const PxF32 *A,const PxF32 *B,const PxF32 *C,PxF32 *n) // returns D
{
	...
	PxF32 mag = PxSqrt((vw_x * vw_x) + (vw_y * vw_y) + (vw_z * vw_z));
...
{
  PxF32 dist = (PxF32)PxSqrt(n[0]*n[0] + n[1]*n[1] + n[2]*n[2]);
  if ( dist > 0.0000001f )
  {
    PxF32 mag = 1.0f / dist;
    n[0]*=mag;
  ...
  MeshAnimation(void)
  {
	...
	const MeshAnimTrack * LocateTrack(const char *name) const
	{
		...
		for (PxI32 i=0; i<mTrackCount; i++)
		{
			...
			if ( physx::string::stricmp(t->GetName(),name) == 0 )
			{
				ret = t;
				break;
			}
...
// convenience helper functions.
  ...
  virtual CommLayer *      createCommLayerTelent(const char *address="LOCALHOST",PxU32 port=23) =  0;
...
MeshImport * loadMeshImporters(const char *directory,physx::Foundation *foundation); // loads the mesh import library (dll) and all available importers from the same directory.
...
// trivial allocator for the 3.0 foundation
class FoundationAllocator : public physx::PxUserAllocator
{

	virtual void* allocate(size_t size, PxU32 handle, const char* filename, int line)
	{
...
class FoundationOutputStream : public physx::PxUserOutputStream
{
public:
	virtual void reportError(PxErrorCode::Enum code, const char* message, const char* file, int line) 
	{
		PX_FORCE_PARAMETER_REFERENCE(code);
		PX_FORCE_PARAMETER_REFERENCE(message);
...
PX_INLINE bool verifyFoundation(physx::Foundation* foundation)
{
	if (&physx::Foundation::getInstance() == NULL)
	{
		if (foundation != NULL)
			physx::Foundation::setInstance(foundation);
		else
		{
			FoundationAllocator* allocator = new FoundationAllocator;
			FoundationOutputStream *outputStream = new FoundationOutputStream;
			Foundation *f = physx::Foundation::createInstance(PX_PUBLIC_FOUNDATION_VERSION, *outputStream, *allocator);
			if ( f == NULL )
				return false;
		}
	}

...
namespace shdfnd2
{
	...
	*/
	PX_FORCE_INLINE PxF32 sign(const PxF32 a)					{	return intrinsics::sign(a); }

	/**
	\brief sign returns the sign of its argument. The sign of zero is undefined.
	*/
	PX_FORCE_INLINE PxF64 sign(const PxF64 a)					{	return (a >= 0.0) ? 1.0 : -1.0;		}

	/**
	\brief sign returns the sign of its argument. The sign of zero is undefined.
	*/
	PX_FORCE_INLINE PxI32 sign(const PxI32 a)					{	return (a >= 0) ? 1 : -1;			}

	/**
	\brief Returns true if the two numbers are within eps of each other.
	*/
	PX_FORCE_INLINE bool equals(const PxF32 a, const PxF32 b, const PxF32 eps){	return (PxAbs(a - b) < eps);	}

	/**
	\brief Returns true if the two numbers are within eps of each other.
	*/
	PX_FORCE_INLINE bool equals(const PxF64 a, const PxF64 b, const PxF64 eps){	return (PxAbs(a - b) < eps);	}

	/**
	\brief The floor function returns a floating-point value representing the largest integer that is less than or equal to x.
	*/
	PX_FORCE_INLINE PxF32 floor(const PxF32 a)					{	return ::floorf(a);					}

	/**
	\brief The floor function returns a floating-point value representing the largest integer that is less than or equal to x.
	*/
	PX_FORCE_INLINE PxF64 floor(const PxF64 a)					{	return ::floor(a);					}

	/**
	\brief The ceil function returns a single value representing the smallest integer that is greater than or equal to x. 
	*/
	PX_FORCE_INLINE PxF32 ceil(const PxF32 a)					{	return ::ceilf(a);	}

	/**
	\brief The ceil function returns a double value representing the smallest integer that is greater than or equal to x. 
	*/
	PX_FORCE_INLINE PxF64 ceil(const PxF64 a)					{	return ::ceil(a);	}
	...
	*/
	PX_FORCE_INLINE PxF32 mod(const PxF32 x, const PxF32 y)			{	return (PxF32)::fmod(x,y);			}
	...
	If the value of y is 0.0, mod returns a quiet NaN.
	*/
	PX_FORCE_INLINE PxF64 mod(const PxF64 x, const PxF64 y)			{	return ::fmod(x,y);					}

	/**
	\brief Square.
	*/
	PX_FORCE_INLINE PxF32 sqr(const PxF32 a)					{	return a*a;		}

	/**
	\brief Square.
	*/
	PX_FORCE_INLINE PxF64 sqr(const PxF64 a)					{	return a*a;	}

	/**
	\brief Calculates x raised to the power of y.
	*/
	PX_FORCE_INLINE PxF32 pow(const PxF32 x, const PxF32 y)			{	return ::powf(x,y);	}

	/**
	\brief Calculates x raised to the power of y.
	*/
	PX_FORCE_INLINE PxF64 pow(const PxF64 x, const PxF64 y)			{	return ::pow(x,y);	}

	/**
	\brief Calculates e^n
	*/
	PX_FORCE_INLINE PxF32 exp(const PxF32 a)					{	return ::exp(a);	}
	/**

	\brief Calculates e^n
	*/
	PX_FORCE_INLINE PxF64 exp(const PxF64 a)					{	return ::exp(a);	}

	/**
	\brief Calculates logarithms.
	*/
	PX_FORCE_INLINE PxF32 logE(const PxF32 a)					{	return ::log(a);	}

	/**
	\brief Calculates logarithms.
	*/
	PX_FORCE_INLINE PxF64 logE(const PxF64 a)					{	return ::log(a);	}

	/**
	\brief Calculates logarithms.
	*/
	PX_FORCE_INLINE PxF32 log2(const PxF32 a)					{   return ::log(a) / 0.693147180559945309417f;	}

	/**
	\brief Calculates logarithms.
	*/
	PX_FORCE_INLINE PxF64 log2(const PxF64 a)					{	return ::log(a) / 0.693147180559945309417;	}

	/**
	\brief Calculates logarithms.
	*/
	PX_FORCE_INLINE PxF32 log10(const PxF32 a)					{	return (PxF32)::log10(a);	}

	/**
	\brief Calculates logarithms.
	*/
	PX_FORCE_INLINE PxF64 log10(const PxF64 a)					{	return ::log10(a);	}

	/**
	\brief Converts degrees to radians.
	*/
	PX_FORCE_INLINE PxF32 degToRad(const PxF32 a)				{	return (PxF32)0.01745329251994329547 * a;	}

	/**
	\brief Converts degrees to radians.
	*/
	PX_FORCE_INLINE PxF64 degToRad(const PxF64 a)				{	return (PxF64)0.01745329251994329547 * a;	}

	/**
	\brief Converts radians to degrees.
	*/
	PX_FORCE_INLINE PxF32 radToDeg(const PxF32 a)				{	return (PxF32)57.29577951308232286465 * a;	}

	/**
	\brief Converts radians to degrees.
	*/
	PX_FORCE_INLINE PxF64 radToDeg(const PxF64 a)				{	return (PxF64)57.29577951308232286465 * a;	}

	//! \brief compute sine and cosine at the same time. There is a 'fsincos' on PC that we probably want to use here
	PX_FORCE_INLINE void sincos(const PxF32 radians, PxF32& sin, PxF32& cos)
	{
		/* something like:
		_asm fld  Local
		_asm fsincos
	...
	PX_FORCE_INLINE PxF32 rand(const PxF32 a, const PxF32 b)	
{
	...
	PX_FORCE_INLINE PxF32 angle(const PxVec3& v0, const PxVec3& v1)
	{
	...
	PX_FORCE_INLINE bool sameDirection(const PxVec3& d, const PxVec3& p)
	{
		return d.dot(p) >= 0.0f;
	}

	PX_INLINE PxVec3 log(const PxQuat& q)
	{
	...
	PX_INLINE PxQuat exp(const PxVec3& v)
	{
		const PxReal m = v.magnitudeSquared();
		return m<1e-24 ? PxQuat::identity() : 
						 PxQuat(PxSqrt(m),v*PxRecipSqrt(m));
	}

	//! Computes the maximum delta to another transform
	PX_INLINE PxReal maxComponentDelta(const PxTransform& t0, const PxTransform& t1)
	{
	...
	*/
	PX_FORCE_INLINE PxU32 largestAxis(const PxVec3& v)
	{
		PxU32 m = v.y > v.x ? 1 : 0;
		return v.z > v[m] ? 2 : m;
	}
	...
	*/
	PX_FORCE_INLINE PxU32 closestAxis(const PxVec3& v)
	{
		PxU32 m = PxAbs(v.y) > PxAbs(v.x) ? 1 : 0;
		return PxAbs(v.z) > PxAbs(v[m]) ? 2 : m;
	}

	PX_INLINE PxU32 closestAxis(const PxVec3& v, PxU32& j, PxU32& k)
	{
	...
	*/
	PX_INLINE void makeFatEdge(PxVec3& p0, PxVec3& p1, PxReal fat_coeff)
	{
		PxVec3 Delta = p1 - p0;
	...
	{
		twist = q.x != 0.0f ? PxQuat(q.x,0,0,q.w).getNormalized() : PxQuat::identity();
	...
	*/
	...
	PX_INLINE PxMat33 rotFrom2Vectors(const PxVec3 &from, const PxVec3 &to)
	{
		...
		if( (from - to).magnitudeSquared() < 1e-4f )
			return PxMat33::identity();

		// Early exit if to = -from
		if( (from + to).magnitudeSquared() < 1e-4f )
			return PxMat33::diagonal(PxVec3(1.0f, -1.0f, -1.0f));

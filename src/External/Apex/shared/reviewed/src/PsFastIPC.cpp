...
public:
	...
	virtual ErrorCode 		 sendMessage(bool bufferIfFull,const char *fmt,...)  // send a message to the other process using a printf style format.
	{
		...
		va_list arg;
		va_start( arg, fmt );
		physx::string::_vsnprintf(wbuff,8191, fmt, arg);
		va_end(arg);
   		PxU32 len = (PxU32 )strlen(wbuff);
	...
	virtual ErrorCode		 sendData(MessageType type,const void *data,PxU32 data_len,bool bufferIfFull)
	{
		ErrorCode ret = EC_OK;
   		...
   		{
   			// TODO
			PxRingBuffer::MessageType mt = (PxRingBuffer::MessageType)type;
			PxRingBuffer::ErrorCode rec = mWriter->sendData(mt,data,data_len,bufferIfFull);
	...
	virtual MessageType receiveData(const void *&data,PxU32 &data_len)
	{
		...
		data_len = 0;
   		...
   		if ( mReader )
   		{
			PxRingBuffer::MessageType rec = mReader->receiveData(data,data_len);
			if ( rec != PxRingBuffer::MT_NONE )
			{
				ret = (MessageType)rec;
			}
    	}

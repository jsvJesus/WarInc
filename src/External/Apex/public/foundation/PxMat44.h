...
#ifndef PX_FOUNDATION_PXMAT44_H
#define PX_FOUNDATION_PXMAT44_H
...
public:
	//! Default constructor
	PX_CUDA_MEMBER PX_INLINE PxMat44()
	{}

	//! Construct from four 4-vectors
	PX_CUDA_MEMBER PxMat44(const PxVec4& col0, const PxVec4& col1, const PxVec4& col2, const PxVec4 &col3)
		: column0(col0), column1(col1), column2(col2), column3(col3)
	{}

	//! Construct from three base vectors and a translation
	PX_CUDA_MEMBER PxMat44(const PxVec3& column0, const PxVec3& column1, const PxVec3& column2, const PxVec3& column3)
		: column0(column0,0), column1(column1,0), column2(column2,0), column3(column3,1)
	{}


	//! Construct from float[9]
	explicit PX_CUDA_MEMBER PX_INLINE PxMat44(PxReal values[]):
	...
	explicit PX_CUDA_MEMBER PX_INLINE PxMat44(const PxQuat& q)
	{
		const PxReal x = q.x;
		const PxReal y = q.y;
		const PxReal z = q.z;
	...
	explicit PX_CUDA_MEMBER PX_INLINE PxMat44(const PxVec4& diagonal):
	...
	PX_CUDA_MEMBER PxMat44(const PxMat33& orientation, const PxVec3& position):
	...
	//! Copy constructor
	PX_CUDA_MEMBER PX_INLINE PxMat44(const PxMat44& other)
		: column0(other.column0), column1(other.column1), column2(other.column2), column3(other.column3)
	{}

	//! Assignment operator
	PX_CUDA_MEMBER PX_INLINE const PxMat44& operator=(const PxMat44& other)
	{
	...
	// Deprecated!
	PX_CUDA_MEMBER PX_INLINE static PxMat44 identity()
	{
		return getIdentity();
	}

	PX_CUDA_MEMBER PX_INLINE static PxMat44 zero()
	{
		return getZero();
	}

	PX_CUDA_MEMBER PX_INLINE static PxMat44 getIdentity()
	{
		return PxMat44(
	...
	PX_CUDA_MEMBER PX_INLINE static PxMat44 getZero()
	{
		return PxMat44(PxVec4(0.0f), PxVec4(0.0f), PxVec4(0.0f), PxVec4(0.0f));
	}

	//! Get transposed matrix
	PX_CUDA_MEMBER PX_INLINE PxMat44 getTranspose() const
	{
	...
	//! Unary minus
	PX_CUDA_MEMBER PX_INLINE PxMat44 operator-() const
	{
		return PxMat44(-column0, -column1, -column2, -column3);
	}

	//! Add
	PX_CUDA_MEMBER PX_INLINE PxMat44 operator+(const PxMat44& other) const
	{
	...
	PX_CUDA_MEMBER PX_INLINE PxMat44 operator*(PxReal scalar) const
	{
	...
	//! Equals-add
	PX_CUDA_MEMBER PX_INLINE PxMat44& operator+=(const PxMat44& other)
	{
		column0 += other.column0;
		column1 += other.column1;
		column2 += other.column2;
	...
	PX_CUDA_MEMBER PX_INLINE PxMat44& operator*=(PxReal scalar)
	{
		column0 *= scalar;
		column1 *= scalar;
		column2 *= scalar;
	...
	PX_CUDA_MEMBER PX_FORCE_INLINE PxReal operator()(unsigned int row, unsigned int col) const
	{
		return (*this)[col][row];
	}

	//! Element access, mathematical way!
	PX_CUDA_MEMBER PX_FORCE_INLINE PxReal& operator()(unsigned int row, unsigned int col)
	{
		return (*this)[col][row];
	}
	
	//! Transform vector by matrix, equal to v' = M*v
	PX_CUDA_MEMBER PX_INLINE PxVec4 transform(const PxVec4& other) const
	{
		return column0*other.x + column1*other.y + column2*other.z + column3*other.w;
	}

	//! Transform vector by matrix, equal to v' = M*v
	PX_CUDA_MEMBER PX_INLINE PxVec3 transform(const PxVec3& other) const
	{
		return transform(PxVec4(other,1)).getXYZ();
	}

	//! Rotate vector by matrix, equal to v' = M*v
	PX_CUDA_MEMBER PX_INLINE PxVec4 rotate(const PxVec4& other) const
	{
		return column0*other.x + column1*other.y + column2*other.z + column3*0;
	}

	//! Rotate vector by matrix, equal to v' = M*v
	PX_CUDA_MEMBER PX_INLINE PxVec3 rotate(const PxVec3& other) const
	{
		return rotate(PxVec4(other,1)).getXYZ();
	}


	PX_CUDA_MEMBER PX_INLINE PxVec3 getBasis(int num) const
	{
		PX_ASSERT(num>=0 && num<=3);
		return (&column0)[num].getXYZ();
	}

	PX_CUDA_MEMBER PX_INLINE PxVec3 getPosition() const
	{
		return column3.getXYZ();
	}

	PX_CUDA_MEMBER PX_INLINE void setPosition(const PxVec3& position)
	{
	...
	PX_CUDA_MEMBER PX_FORCE_INLINE const PxReal* front() const
	{
		return &column0.x;
	}

	PX_CUDA_MEMBER PX_FORCE_INLINE			PxVec4& operator[](int num)			{return (&column0)[num];}
	PX_CUDA_MEMBER PX_FORCE_INLINE const	PxVec4& operator[](int num)	const	{return (&column0)[num];}

	PX_CUDA_MEMBER PX_INLINE void	scale(const PxVec4& p)
	{
		column0 *= p.x;
		column1 *= p.y;
		column2 *= p.z;
	...
	PX_CUDA_MEMBER PX_INLINE PxMat44 inverseRT(void) const
	{

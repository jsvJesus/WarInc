#ifndef __PX_CUDA_CONTEXT_MANAGER__
#define __PX_CUDA_CONTEXT_MANAGER__
...
public:
     ...
     *
     * If left NULL, the CudaContextManager will create a new context.
     * If graphicsDevice is also not NULL, this new CUDA context will be
     * bound to that graphics device, enabling the use of CUDA/Graphics
     * interop features.
     *
     * If ctx is not NULL, the specified context must be applied to the
     * thread that is allocating the CudaContextManager at creation
     * time (aka, it cannot be popped).  The CudaContextManager will
     * take ownership of the context until the manager is released.  All
     * access to the context must be gated by lock acquisition.
     *
     * If the user provides a context for the CudaContextManager, the
     * context _must_ have either been created on the GPU ordinal
     * returned by getSuggestedCudaDeviceOrdinal() or on your graphics
     * device.
     *
     * It is perfectly acceptable to allocate device or host pinned
     * memory from the context outside the scope of the
     * CudaMemoryManager, so long as you manage it's eventual cleanup.
     */
	CUcontext            *ctx;
     ...
     *
     * Only applicable when ctx is NULL, thus forcing a new context to
     * be created.  In that case, the created context will be bound to
     ...
     *
     * If ctx is NULL, this value describes the nature of the
     * graphicsDevice pointer provided by the user.  Else it describes
     * the nature of the context provided by the user.
     */
     ...
     * CudaContextManager is released.  Size is in bytes, has to be
     * power of two and bigger than the page size.  Set to 0 to only use
     * dynamic pages.
     *
     * Note: On Vista O/S and above, there is a per-memory allocation
     * overhead to every CUDA work submission, so we recommend that you
     * carefully tune this initial base memory size to closely
     * approximate the ammount of memory your application will consume.
     */
     ...
     *
     * The memory manager will dynamically grow and shrink in blocks
     * multiple of this page size. Size has to be power of two and
     * bigger than 0.
     */
 ...
 *
 * A CudaContextManager manages access to a single CUDA context,
 * allowing it to be shared between multiple scenes.   Memory
 * allocations are dynamic: starting with an initial heap size and
 * growing on demand by a configurable page size.  The context must be
 * acquired from the manager before using any CUDA APIs.
 *
 * The CudaContextManager is based on the CUDA driver API and explictly
 * does not support the the CUDA runtime API (aka, CUDART).
 *
 * To enable CUDA use by an APEX scene, a CudaContextManager must be
 * created (supplying your own CUDA context, or allowing a new context
 * to be allocated for you), the GpuDispatcher for that context is
 * retrieved via the getGpuDispatcher() method, and this is assigned to
 * the TaskManager that is given to the scene via it's NxApexSceneDesc.
 *
 * (example pseudo code)
 *
 * using namespace pxtask;
 *
 * tm = TaskManager::createTaskManager();
 * CudaContextManagerDesc desc;
 * ccm = apexSdk->createCudaContextManager( desc );
 * if( ccm->contextIsValid() )
 *    tm->setGpuDispatcher( ccm->getGpuDispatcher() )
 * else
 *    ccm->release();
 * NxApexSceneDesc adesc;
 * adesc.taskManager = tm;
 * apexSdk->createScene( adesc );
 *
 *
 */
...
public:
     ...
     * especially if the manager was responsible for allocating it's own
    ...
    virtual bool supportsArchSM13() const = 0;  //!< GT200
	...
	*/
     ...
     *
     * When the manager instance is released, it also releases it's
     ...
     * using it's GpuDispatcher.  Those scenes must be released first
...
{
public:
    ScopedCudaLock( CudaContextManager& ctx ) : mCtx(&ctx) { mCtx->acquireContext(); }
    ~ScopedCudaLock()									   { mCtx->releaseContext(); }
 ...
 * CUDA tasks.
 */
CudaContextManager* createCudaContextManager( const CudaContextManagerDesc& desc );

/**
 * Unload CUDA support from pxtask
 */
void                unloadCudaSupprt();

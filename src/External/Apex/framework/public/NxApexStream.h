...
PX_INLINE physx::PxFileBuf& operator >> (physx::PxFileBuf& stream, physx::PxMat33Legacy& m)
{
	stream >> m(0, 0) >> m(0, 1) >> m(0, 2) >> m(1, 0) >> m(1, 1) >> m(1, 2) >> m(2, 0) >> m(2, 1) >> m(2, 2);
	return stream;
}
PX_INLINE physx::PxFileBuf& operator >> (physx::PxFileBuf& stream, physx::PxMat34Legacy& tm)
{
	stream >> tm.M >> tm.t;
	return stream;
}
...
PX_INLINE physx::PxFileBuf& operator >> (physx::PxFileBuf& stream, NxGroupsMask& gm)
{
	stream >> gm.bits0 >> gm.bits1 >> gm.bits2 >> gm.bits3;
	return stream;
}

PX_INLINE physx::PxFileBuf& operator >> (physx::PxFileBuf& stream, NxRange<physx::PxF32>& r)
{
	stream >> r.minimum >> r.maximum;
	return stream;
}
PX_INLINE physx::PxFileBuf& operator >> (physx::PxFileBuf& stream, NxRange<physx::PxU32>& r)
{
	stream >> r.minimum >> r.maximum;
	return stream;
}
PX_INLINE physx::PxFileBuf& operator >> (physx::PxFileBuf& stream, NxRange<physx::PxVec3>& r)
{
	stream >> r.minimum >> r.maximum;
	return stream;
}
PX_INLINE physx::PxFileBuf& operator >> (physx::PxFileBuf& stream, NxRenderCullMode::Enum& w)
{
...
PX_INLINE physx::PxFileBuf& operator << (physx::PxFileBuf& stream, const NxGroupsMask gm)
{
	stream << gm.bits0 << gm.bits1 << gm.bits2 << gm.bits3;
	return stream;
}
PX_INLINE physx::PxFileBuf& operator << (physx::PxFileBuf& stream, const physx::PxMat33Legacy& m)
{
	stream << m(0, 0) << m(0, 1) << m(0, 2) << m(1, 0) << m(1, 1) << m(1, 2) << m(2, 0) << m(2, 1) << m(2, 2);
	return stream;
}
PX_INLINE physx::PxFileBuf& operator << (physx::PxFileBuf& stream, const physx::PxMat34Legacy& tm)
{
	stream << tm.M << tm.t;
	return stream;
}
...
PX_INLINE physx::PxFileBuf& operator << (physx::PxFileBuf& stream, const NxRange<physx::PxF32> r)
{
	stream << r.minimum << r.maximum;
	return stream;
}
PX_INLINE physx::PxFileBuf& operator << (physx::PxFileBuf& stream, const NxRange<physx::PxU32> r)
{
	stream << r.minimum << r.maximum;
	return stream;
}
PX_INLINE physx::PxFileBuf& operator << (physx::PxFileBuf& stream, const NxRange<physx::PxVec3> r)
{
	stream << r.minimum << r.maximum;
	return stream;
}

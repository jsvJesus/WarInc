...
void Gu::HeightField::exportExtraData(PxSerialStream& stream)
{
	...
	Cm::alignStream(stream, 16);	// PT: explicit align because we used the aligned allocator here
#else
	const PxU32 size = mData.rows * mData.columns * sizeof(PxHeightFieldSample);
	Cm::alignStream(stream, PX_SERIAL_DEFAULT_ALIGN_EXTRA_DATA_WIP);	// PT: generic align within the generic allocator
#endif

...
namespace Sq
{
	...
	*/
	...
	class Pruner : public PruningPool
	{
		...
		// flushUpdates() can only be called by one thread at a time, *however* it may be called 
		// while another thread is performing raycasts (reads) and must not update state in this case.
		// To ensure this, the pruner should perform a complete update the first time flushUpdates() is called
		// and on subsequent calls it should early exit. This relies on the fact that it is illegal for
		// users to perform API write operations while a query is running.

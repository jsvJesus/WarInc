...
void DynamicPruner2::flushUpdates()
{
	// Note: it is not safe to call DynamicPruner2::eagerUpdatePruningTrees() here
	...
	// Calling StaticPruner::eagerUpdatePruningTrees() below is safe because  
	// it will early exit if the tree is already built, if it is not already  
	// built then we must be the first thread performing raycasts on a dirty  
	// tree and other scene query threads will be locked out by the write lock 
	// in SceneQueryManager::flushUpdates()

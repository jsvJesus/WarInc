...
void SceneQueryManager::cull(PruningTemps& temps, CulledObjects& objects, const Gu::Plane* planes, PxU32 nb_planes, PxU32 flags) const
{
	PX_ASSERT(planes);
...
void SceneQueryManager::stab(StabCallback callback, void* user_data, const PxVec3& orig, const PxVec3& dir, float max_dist, PxU32 flags) const
{
	PX_ASSERT(callback);
...
void SceneQueryManager::overlap(ReportPrunablesCallback cb, void* userData, const Gu::Sphere& sphere, PxU32 flags) const
{
...
void SceneQueryManager::overlap(ReportPrunablesCallback cb, void* userData, const PxBounds3& box, PxU32 flags) const
{
//	const bool first_contact = (flags&PQF_FIRST_CONTACT)!=0;
	const bool first_contact = false;
	for(PxU32 i=0;i<2;i++)
...
void SceneQueryManager::overlap(ReportPrunablesCallback cb, void* userData, const Gu::Box& box, PxU32 flags) const
{
//	const bool first_contact = (flags&PQF_FIRST_CONTACT)!=0;
	const bool first_contact = false;
	for(PxU32 i=0;i<2;i++)
...
void SceneQueryManager::overlap(ReportPrunablesCallback cb, void* userData, const Gu::Capsule& capsule, PxU32 flags) const
{
//	const bool first_contact = (flags&PQF_FIRST_CONTACT)!=0;
	const bool first_contact = false;
	for(PxU32 i=0;i<2;i++)
...
SceneQueryShapeData* SceneQueryManager::addShape(const NpShape& shape, bool dynamic)
{
	...
	Mutex::ScopedLock lock(mSceneQueryLock);
	AddObject(prn);
...
void SceneQueryManager::flushShapes() const
{
...
void SceneQueryManager::flushUpdates() const
{

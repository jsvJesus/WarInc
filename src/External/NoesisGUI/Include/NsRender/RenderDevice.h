...
public:
    ...
    /// Begins a block for uploading texture data. All calls to UpdateTexture() are enclosed
    /// between BeginUpdatingTextures() and EndUpdatingTextures(). This block is guaranteed
    /// to be executed strictly outside any render pass.
    virtual void BeginUpdatingTextures();
    ...
    /// per texture. It is always invoked between BeginUpdatingTextures() and EndUpdatingTextures(),
    /// and is guaranteed to happen strictly outside any render pass.
    ...
    /// Marks the end of a texture update block, providing the list of textures that were
    /// modified. This indicates they are now ready to be used in upcoming render passes.
    /// Implementations may perform necessary synchronization or state transitions
    /// (e.g., from copy/write to shader-read). Like BeginUpdatingTextures(), this function
    /// is guaranteed to be called strictly outside any render pass.
    virtual void EndUpdatingTextures(Texture** textures, uint32_t count);
    ...
    /// the surface must be discarded and replaced with arbitrary data. Surface must not be cleared.
    ...
    /// Resolves multisample render target. Transient surfaces (stencil, colorAA) must be discarded.
    /// Only the specified list of surface regions must be resolved
    ...
    /// These validation helpers are useful when building pipeline state objects (PSOs),
    /// as they allow you to skip invalid combinations and reduce the total number of
    /// permutations that need to be created
    static bool IsValidState(Shader shader, RenderState state);
    static bool IsValidBlendMode(Shader shader, BlendMode::Enum blendMode);
    static bool IsValidStencilMode(Shader shader, StencilMode::Enum stencilMode);
    static bool IsValidColorEnable(Shader shader, bool colorEnable);
    static bool IsValidWireframe(Shader shader, bool wireframe);

...
public:
    ...
    static void SealConditions(ConditionCollection* conditions, bool sealProperty);

    /// Gets or sets the binding that produces the property value of the data object. This is only
    /// applicable to MultiDataTrigger objects
    //@{
    ...
    /// Gets or sets the value of the condition. If Value is DependencyProperty.UnsetValue this
    /// condition is not valid and will be ignored
    //@{
    BaseComponent* GetValue() const;
    void SetValue(BaseComponent* object);
    //@}

    /// Seals this condition. When 'sealProperty' is true, it checks that Property and its Value
    /// are valid
    void Seal(bool sealProperty);

    /// Indicates if Condition is sealed
    bool IsSealed() const;

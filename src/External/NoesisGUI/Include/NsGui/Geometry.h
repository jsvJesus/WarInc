...
public:
    ...
    /// Appends the transformed SVG-path commands for this geometry to the buffer
    virtual void GetCommands(BaseVector<uint32_t>& commands, const Transform2& mtx);
...
protected:
    ...
    virtual Rect GetRenderBoundsOverride(Pen* pen, Transform* transform) const;

    /// Implements contain tests
    //@{
    virtual bool FillContainsOverride(const Point& p, Transform* transform) const;
    virtual bool StrokeContainsOverride(Pen* pen, const Point& p, Transform* transform) const;
    //@}

    static Rect GetRenderBounds(Geometry* geometry, Pen* pen, Transform* transform);
    static bool FillContains(Geometry* geometry, const Point& p, Transform* transform);
    static bool StrokeContains(Geometry* geometry, Pen* pen, const Point& p, Transform* transform);

    /// Called when the geometry transform is updated

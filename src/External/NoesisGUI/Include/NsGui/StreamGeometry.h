...
public:
    ...
    /// Opens a StreamGeometryContext for defining the geometry using drawing commands
    StreamGeometryContext Open();

    /// Sets the command buffer that encodes the geometry's path instructions. This provides an
    /// alternative to using 'Open()' and manually writing to the StreamGeometryContext
    void SetCommandBuffer(ArrayRef<uint32_t> commands);

    /// Returns an iterator of commands describing the path
    PathCommandIterator GetCommands() const;

    /// Returns the command buffer that encodes the geometry's path instructions
    ArrayRef<uint32_t> GetCommandBuffer() const;
    ...
    void GetCommands(BaseVector<uint32_t>& commands, const Transform2& mtx) override;
    //@}
...
protected:
    ...
    Rect GetRenderBoundsOverride(Pen* pen, Transform* transform) const override;
    bool FillContainsOverride(const Point& point, Transform* transform) const override;
    bool StrokeContainsOverride(Pen* pen, const Point& point, Transform* transform) const override;
    //@}

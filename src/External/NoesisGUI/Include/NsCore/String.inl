...
inline void BaseString::AppendFormatVA(const char* format, va_list* args)
{
  ...
  #ifdef vsnprintf
    // Ensure we use the standard 'vsnprintf'
    // On some platforms (e.g., Windows), client code may redefine 'vsnprintf' to '_vsnprintf',
    // which behaves differently (e.g., does not null-terminate on overflow)
    // We temporarily undefine any macro to safely call the standard function
    #undef vsnprintf
  #endif
...
inline void BaseString::Assign(BaseString&& str, uint32_t capacity)
{
    if (this != &str)
    {
        // If str is not using inlined memory steals its buffer
        if (!str.mIsSmall)
        {
            if (!mIsSmall)
            {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<unsigned N>
inline FixedString<N>::FixedString(const BaseString& str): BaseString(N - 1)
{
    mSmallString[0] = 0;
    Append(str);
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<unsigned N>
inline FixedString<N>::FixedString(BaseString&& str): BaseString(N - 1)
{
    mSmallString[0] = 0;
    Assign(MoveArg(str));
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<unsigned N>
inline FixedString<N>& FixedString<N>::operator=(const BaseString& str)
{
    Assign(str);
    return *this;
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<unsigned N>
inline FixedString<N>& FixedString<N>::operator=(BaseString&& str)
{
    Assign(MoveArg(str));
    return *this;
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<unsigned N>
inline void FixedString<N>::ClearShrink()
{
    this->Clear();
    ...
    {
        Dealloc(this->Begin());
        this->ResetToSmall(N - 1);
    }
}

////////////////////////////////////////////////////////////////////////////////////////////////////
template<unsigned N>
inline bool FixedString<N>::IsSmall() const
{
...
{
    static bool IsEmpty(const BaseString& key) { return key.Size() == 0xFFFFFFFF; }
    static void MarkEmpty(BaseString& key) { *(uint32_t*)&key = 0xFFFFFFFF; }
    static uint32_t HashValue(const BaseString& key) { return StrHash(key.Str()); }
    static uint32_t HashValue(const char* key) { return StrHash(key); }
    static bool IsEqual(const BaseString& l, const BaseString& r) { return !IsEmpty(l) && l == r; }
    static bool IsEqual(const BaseString& l, const char* r) { return !IsEmpty(l) && l == r; }
};
...
struct CaseStringHashKeyInfo
{
    static bool IsEmpty(const BaseString& key) { return key.Size() == 0xFFFFFFFF; }
    static void MarkEmpty(BaseString& key) { *(uint32_t*)&key = 0xFFFFFFFF; }
    static uint32_t HashValue(const BaseString& key) { return StrCaseHash(key.Str()); }
    static uint32_t HashValue(const char* key) { return StrCaseHash(key); }

    static bool IsEqual(const BaseString& l, const BaseString& r)
    {
        return !IsEmpty(l) && l.Size() == r.Size() && StrCaseEquals(l.Str(), r.Str());
    }

    static bool IsEqual(const BaseString& l, const char* r)
    {
...
struct HashBucket<FixedString<N>, Value>
{
    typedef HashBucket_KHV<FixedString<N>, Value, StringHashKeyInfo> Type;
};

template<unsigned N>
struct SetBucket<FixedString<N>>
{
    typedef HashBucket_KH<FixedString<N>, StringHashKeyInfo> Type;
};

...
///  ----------------------------------------------------------------------------------------
    ...
    #define NS_INTERNAL_ASSERT(expr, ...) \
        NS_MACRO_BEGIN \
            if (NS_UNLIKELY(!(expr))) \
            { \
                if (Noesis::InvokeAssertHandler(__FILE__, __LINE__, #expr, ##__VA_ARGS__)) \
                { \
                    NS_DEBUG_BREAK; \
                } \
            } \

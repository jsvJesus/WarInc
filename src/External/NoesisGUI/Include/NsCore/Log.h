...
NS_CORE_KERNEL_API void InvokeLogHandler(const char* file, uint32_t line, uint32_t level,
    const char* channel, NS_FORMAT_PRINTF const char* format, ...)
    NS_FORMAT_PRINTF_ATTR(5);
NS_CORE_KERNEL_API void InvokeLogHandler2(const char* file, uint32_t line, uint32_t level,
    const char* channel, const BaseComponent* obj, NS_FORMAT_PRINTF const char* format, ...)
    NS_FORMAT_PRINTF_ATTR(6);

#define NS_LOG_(level, ...) \
    Noesis::InvokeLogHandler(__FILE__, __LINE__, level, NS_LOG_CHANNEL, __VA_ARGS__)
#define NS_LOG_OBJ_(level, obj, ...) \
    Noesis::InvokeLogHandler2(__FILE__, __LINE__, level, NS_LOG_CHANNEL, obj, __VA_ARGS__)

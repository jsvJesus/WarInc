...
void D3D12RenderContext::BeginRender()
{
    ...
    void* data;
    if (SUCCEEDED(mTimeStampResults->Map(0, &range, &data)))
    {
...
void D3D12RenderContext::SetDefaultRenderTarget(uint32_t, uint32_t, bool doClearColor)
{
    ...
    else
    {
        ...
        mCommands->ClearRenderTargetView(rtv, RGBA, 0, nullptr);
    }
  #endif
...
void D3D12RenderContext::Swap()
{
  ...
  #else
    HRESULT hr = mSwapChain->Present(mSyncInterval, mPresentFlags);
...
void D3D12RenderContext::CreateSwapChain(void* window, bool vsync)
{
    ...
    V(factory->CreateSwapChainForComposition(mQueue, &desc, NULL, &swapChain));
    V(swapChain->QueryInterface(DX_ARGS(&mSwapChain)));
    DX_RELEASE(swapChain);

    IDCompositionVisual* visual = nullptr;
    V(DCompositionCreateDevice(NULL, DX_ARGS(&mCompositionDevice)));

...
#define V(exp) \
    NS_MACRO_BEGIN \
        VkResult err_ = (exp); \
        NS_ASSERT(err_ == VK_SUCCESS, "err = %d", err_); \
    NS_MACRO_END
...
void VKRenderContext::Init(void* window, uint32_t& samples, bool vSync, bool sRGB)
{
  ...
  #ifdef NS_PLATFORM_WINDOWS_DESKTOP
    // It seems not all drivers support WS_EX_NOREDIRECTIONBITMAP
    // Let's downgrade the window handle for now
...
void VKRenderContext::BeginRender()
{
    ...
    if (mHasTimeStampQueries && mFrameNumber >= FramesInFlight)
    {
        ...
        if (r == VK_SUCCESS)
        {
            ...
            mGPUTimeMs = (mTimeStampPeriod * (float(data[2] - data[0]))) / 1000000.0f;
        }
    }
  #endif

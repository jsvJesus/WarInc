...
#define V(exp) \
    NS_MACRO_BEGIN \
        NS_ASSERT(glGetError() == GL_NO_ERROR); \
        (exp); \
        NS_ASSERT(glGetError() == GL_NO_ERROR); \
    NS_MACRO_END
...
////////////////////////////////////////////////////////////////////////////////////////////////////
const char* GLRenderContext::ShaderLang() const
{
  #if NS_RENDERER_OPENGL
    return "glsl";
  #else
...
void GLRenderContext::Init(void* window, uint32_t& sampleCount, bool vsync, bool sRGB)
{
    ...
    sampleCount = Min((GLsizei)sampleCount, maxSamples);
    ...
    @autoreleasepool
    {
        ...
        // This fixes a bug in macOS 10.14 and Xcode 10 (seems to be fixed in macOS 10.14.2)
    ...
    if (strstr(extensions, "WGL_ARB_pixel_format") && strstr(extensions, "WGL_ARB_multisample"))
    {
        ...
        do
        {
            ...
            UINT n;
            if (!wglChoosePixelFormatARB(mSurface, attribs, NULL, 1, &mPixelFormat, &n) || n == 0)
            {
                mPixelFormat = 0;
                samples >>= 1;
            }
        }
        while (mPixelFormat == 0 && samples != 0);

        if (mPixelFormat != 0)
        {
            int attrib = WGL_SAMPLES_ARB;
            int value;
            BOOL r = wglGetPixelFormatAttribivARB(mSurface, mPixelFormat, 0, 1, &attrib, &value);
            NS_ASSERT(r);
    ...
    if (mPixelFormat == 0)
    {
        ...
        pfd.cStencilBits = 8;
        pfd.cColorBits = 24;
        mPixelFormat = ChoosePixelFormat(mSurface, &pfd);

        sampleCount = 1;
    }

    NS_ASSERT(mPixelFormat != 0);

    PIXELFORMATDESCRIPTOR pfd;
    BOOL r = DescribePixelFormat(mSurface, mPixelFormat, sizeof(pfd), &pfd);
    NS_ASSERT(r);
    ...
    r = SetPixelFormat(mSurface, mPixelFormat, &pfd);
    NS_ASSERT(r);
    ...
    // For WS_EX_NOREDIRECTIONBITMAP we need WGL_NV_DX_interop (not always available)
    // For now, let's request a window handle without this optimization flag
    LONG style = GetWindowLong((HWND)window, GWL_EXSTYLE);
    if (style & WS_EX_NOREDIRECTIONBITMAP)
    {
        #define WM_DOWNGRADE_WINDOW_HANDLE WM_USER + 0x4322
        SendMessage((HWND)window, WM_DOWNGRADE_WINDOW_HANDLE, 0, 0);
    }
...
#ifdef NS_DEBUG
    if (StrFindFirst(extensions, "EGL_KHR_create_context") != -1)
    {
        attribs[2] = EGL_CONTEXT_FLAGS_KHR;
        attribs[3] = EGL_CONTEXT_OPENGL_DEBUG_BIT_KHR;
    }
#else
    if (StrFindFirst(extensions, "EGL_KHR_create_context_no_error") != -1)
    {
...
#if EGL_ANDROID_get_frame_timestamps
    eglGetNextFrameIdANDROID = 0;
    if (StrFindFirst(extensions, "ANDROID_get_frame_timestamps") != -1)
    {
        mReadQuery = 0;
        mWriteQuery = 0;
        lastRenderCompleteTime = 0;
        GL_IMPORT(false, PFNEGLGETNEXTFRAMEIDANDROIDPROC, eglGetNextFrameIdANDROID);
        GL_IMPORT(false, PFNEGLGETFRAMETIMESTAMPSANDROIDPROC, eglGetFrameTimestampsANDROID);
    }
#endif
...
#ifdef NS_DEBUG
    // Older EGL drivers may be not compatible with EGL_CONTEXT_OPENGL_DEBUG_BIT_KHR
    ...
    NS_ASSERT(mContext != EGL_NO_CONTEXT);
    ...
    mDevice = GLFactory::CreateDevice(false);
}
...
void GLRenderContext::SetWindow(void* window)
{
    ...
    if (window)
    {
        ...
        NS_ASSERT(mSurface != EGL_NO_SURFACE);
      ...
      #if defined(NS_PROFILE) && EGL_ANDROID_get_frame_timestamps
        if (HaveFrameTimestamps())
        {
            EGL_V(eglSurfaceAttrib(mDisplay, mSurface, EGL_TIMESTAMPS_ANDROID, EGL_TRUE));
        }
      #endif
    }
  #endif
...
void GLRenderContext::BeginRender()
{
    ...
    if (HaveFrameTimestamps())
    {
        EGLBoolean r = eglGetNextFrameIdANDROID(mDisplay, mSurface, &mFrames[mWriteQuery]);
        NS_ASSERT(r == EGL_TRUE);
    }
    else
  #endif
    if (HaveQueries())
...
void GLRenderContext::EndRender()
{
    ...
    if (HaveFrameTimestamps())
    {
        mWriteQuery = (mWriteQuery + 1) % NS_COUNTOF(mFrames);
        if (mWriteQuery == mReadQuery)
        {
            mReadQuery = (mReadQuery + 1) % NS_COUNTOF(mFrames);
            ...
            if (eglGetFrameTimestampsANDROID(mDisplay, mSurface, mFrames[mReadQuery], 2, timestamps, values))
            {
                EGLnsecsANDROID t = (values[1] - Max(values[0], lastRenderCompleteTime));
                mGPUTime = (float)((double)Max(t, EGLnsecsANDROID(0)) / 1.0e6);
                lastRenderCompleteTime = values[1];
            }
        }
    }
    else
...
////////////////////////////////////////////////////////////////////////////////////////////////////
Noesis::Ptr<NoesisApp::Image> GLRenderContext::CaptureRenderTarget(RenderTarget* surface) const
{
    NS_UNUSED(surface);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void* GLRenderContext::CreatePixelShader(const char* label, uint8_t shader, ArrayRef<uint8_t> code)
{
    return GLFactory::CreatePixelShader(mDevice, label, shader, (const char*)code.Data());
}
...
{
    const char* source_ = GetSource(source);
    const char* type_ = GetType(type);
    const char* severity_ = GetSeverity(severity);
    ...
    if (n > 0)
    {
        ...
        {
            NS_LOG_TRACE("(%s|%s|%s) %.*s", source_, type_, severity_, n, message);
        }
        else
        {
            NS_LOG_DEBUG("(%s|%s|%s) %.*s", source_, type_, severity_, n, message);
        }
    }
}
#endif
...
void GLRenderContext::FindExtensions()
{
    ...
    if (glVersion >= NS_GL_VER(3,0))
    {
        ...
        for (GLint i = 0; i < numExtensions; i++)
        {
            ...
            {
                if (StrEquals(glExtension, extensions[j].name))
                {
    ...
    else
    {
        ...
        if (glExtensions != 0)
        {
            ...
            {
                if (StrFindFirst(glExtensions, extensions[i].name) != -1)
                {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
bool GLRenderContext::HaveFrameTimestamps() const
{
#if EGL_ANDROID_get_frame_timestamps
    return eglGetNextFrameIdANDROID != 0;
#else
    return false;
#endif
}

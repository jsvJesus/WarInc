...
NS_IMPLEMENT_REFLECTION(NumericUpDown, "UserControls.NumericUpDown")
{
    ...
    {
        NumericUpDown* control = DynamicCast<NumericUpDown*>(d);
        if (control != nullptr)
        {
        ...
        CoerceValueCallback(
    [](const DependencyObject* d, const void* value, void* coercedValue) -> bool
    {
        int maxValue = d->GetValue<int>(MaxValueProperty);
        int minValue = d->GetValue<int>(MinValueProperty);

...
#define V(exp) \
    NS_MACRO_BEGIN \
        EMSCRIPTEN_RESULT hr_ = (exp); \
        NS_ASSERT(hr_ == EMSCRIPTEN_RESULT_SUCCESS); \
    NS_MACRO_END


////////////////////////////////////////////////////////////////////////////////////////////////////
EmscriptenDisplay::EmscriptenDisplay(): mWidth(0), mHeight(0), mPixelRatio(1.0f), mLastMouseDown(0)
{
    FillKeyTable();
}
...
void EmscriptenDisplay::UpdateSize()
{
    ...
    V(emscripten_get_element_css_size("#canvas", &width, &height));

    mPixelRatio = (float)ratio;
    mWidth = (uint32_t)(width * ratio);
    mHeight = (uint32_t)(height * ratio);
    V(emscripten_set_canvas_element_size("#canvas", mWidth, mHeight));
...
void EmscriptenDisplay::Show()
{
...
#if __EMSCRIPTEN_major__ * 100 + __EMSCRIPTEN_minor__ < 139
    V(emscripten_set_resize_callback(0, this, false,
#else
    V(emscripten_set_resize_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, this, false,
#endif
    [](int, const EmscriptenUiEvent*, void* user) -> EM_BOOL
    {
    ...
    V(emscripten_set_focusin_callback("#canvas", this, true,
    [](int, const EmscriptenFocusEvent*, void* user) -> EM_BOOL
    {
        EmscriptenDisplay* display = (EmscriptenDisplay*)user;
        if (!display->mIsActive)
        {
            display->mActivated(display);
            display->mIsActive = true;
        }
        return false;
    }));

    V(emscripten_set_focusout_callback("#canvas", this, true,
    [](int, const EmscriptenFocusEvent*, void* user) -> EM_BOOL
    {
        EmscriptenDisplay* display = (EmscriptenDisplay*)user;
        display->mDeactivated(display);
        display->mIsActive = false;
        return false;
    }));

    #define X(e) int((float)(e)->targetX * display->mPixelRatio)
    #define Y(e) int((float)(e)->targetY * display->mPixelRatio)

    V(emscripten_set_mousedown_callback("#canvas", this, true,
    [](int, const EmscriptenMouseEvent* e, void* user) -> EM_BOOL
    {
        EmscriptenDisplay* display = (EmscriptenDisplay*)user;
        uint64_t ticks = HighResTimer::Ticks();

        if (HighResTimer::Seconds(ticks - display->mLastMouseDown) < 0.5f)
        {
            display->mMouseDoubleClick(display, X(e), Y(e), NoesisButton(e->button));
        }
        else
        {
            display->mMouseButtonDown(display, X(e), Y(e), NoesisButton(e->button));
        }

        display->mLastMouseDown = ticks;
        return false;
    }));

    V(emscripten_set_mouseup_callback("#canvas", this, true,
    [](int, const EmscriptenMouseEvent* e, void* user) -> EM_BOOL
    {
        EmscriptenDisplay* display = (EmscriptenDisplay*)user;
        display->mMouseButtonUp(display, X(e), Y(e), NoesisButton(e->button));
        return false;
    }));

    V(emscripten_set_mousemove_callback("#canvas", this, true,
    [](int, const EmscriptenMouseEvent* e, void* user) -> EM_BOOL
    {
        EmscriptenDisplay* display = (EmscriptenDisplay*)user;
        display->mMouseMove(display, X(e), Y(e));
        return false;
    }));

    V(emscripten_set_wheel_callback("#canvas", this, true,
    [](int, const EmscriptenWheelEvent* e, void* user) -> EM_BOOL
    {
        EmscriptenDisplay* display = (EmscriptenDisplay*)user;
        display->mMouseWheel(display, X(&(e->mouse)), Y(&(e->mouse)), (int)(e->deltaY * -60.0));
        return false;
    }));

    V(emscripten_set_keydown_callback("#canvas", this, true,
    [](int, const EmscriptenKeyboardEvent* e, void* user) -> EM_BOOL
    {
        if (e->keyCode <= 0xff)
        {
            EmscriptenDisplay* display = (EmscriptenDisplay*)user;
            ...
            if (key != Key_None)
            {
                ...
                return key == Key_Back || key == Key_Tab || key == Key_Left || key == Key_Right ||
    ...
    V(emscripten_set_keyup_callback("#canvas", this, true,
    [](int, const EmscriptenKeyboardEvent* e, void* user) -> EM_BOOL
    {
        if (e->keyCode <= 0xff)
        {
    ...
    V(emscripten_set_keypress_callback("#canvas", this, true,
    [](int, const EmscriptenKeyboardEvent* e, void* user) -> EM_BOOL
    {
        EmscriptenDisplay* display = (EmscriptenDisplay*)user;

...
public:
    ...
    /// Sets the value of an input in the Rive scene. If a path is provided it indicates the name
    /// of the nested Artboard where the input should be found
    void SetInputValue(const char* path, const char* name, Noesis::BaseComponent* value);

    /// Fires a trigger input in the Rive scene
    void FireInputTrigger(const char* path, const char* name);
    ...
    void SetRunText(const char* path, const char* name, const char* text);
...
private:
    ...
    typedef Noesis::HashMap<Noesis::String, Noesis::Vector<rive::TextValueRun*>> RunsCache;
    RunsCache::Iterator CacheRuns(const char* path, rive::ArtboardInstance* artboard);

...
#ifndef USE_CHOREOGRAPHER
  // Pumps frames using the Choreographer
  // https://developer.android.com/ndk/reference/group/choreographer
  #define USE_CHOREOGRAPHER 1
#endif
...
AndroidDisplay::AndroidDisplay(): mWidth(0), mHeight(0), mHasFocus(false), mIsVisible(false),
    mHasWindow(false), mChoreographerEnabled(false), mChoreographerStarted(false)
{
...
void AndroidDisplay::EnterMessageLoop(bool /*runInBackground*/)
{
    ...
    if (android != nullptr)
    {
        #define ANDROID_IMPORT(x) x = (x##T)dlsym(android, #x)
        ANDROID_IMPORT(AChoreographer_getInstance);
        ANDROID_IMPORT(AChoreographer_postFrameCallback);
        mChoreographerEnabled = AChoreographer_getInstance && AChoreographer_postFrameCallback;
        ChoreographerStart();
    }
#endif
        ...
        while (true)
        {
            ...
            if (mApp->window != nullptr)
            {
                ...
                if (mWidth != width || mHeight != height)
                {
                    mWidth = width;
                    mHeight = height;
                    NS_LOG_TRACE("ANativeWindow size changed to %d x %d (scale=%2.2f)", width,
                        height, GetScale());
            ...
            if (IsReadyToRender() && !mChoreographerEnabled)
            {
                mRender(this);
            }
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void AndroidDisplay::OpenUrl(const char* url)
{
    // Attach current thread to the JavaVM
    JNIEnv* env;
    ...
    env->CallVoidMethod(mApp->activity->clazz, startActivity, intent);
...
float AndroidDisplay::GetScale() const
{
    ...
    return density == ACONFIGURATION_DENSITY_DEFAULT ? 1.0f : (float)density / 160.0f;
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
int AndroidDisplay::LooperTimeout() const
{
    return IsReadyToRender() && !mChoreographerEnabled ? 0 : -1;
}
...
bool AndroidDisplay::ProcessMessages()
{
    ...
    while (ALooper_pollOnce(LooperTimeout(), nullptr, &events, (void**)&source) >= 0)
    {
        if (source != nullptr)
        {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void AndroidDisplay::ChoreographerCallback(long frameTimeNanos, void* data)
{
    AndroidDisplay* this_ = (AndroidDisplay*)data;

    if (this_->IsReadyToRender())
    {
        this_->mRender(this_);
        this_->ChoreographerEnqueue();
    }
    else
    {
        NS_LOG_TRACE("= Choreographer stopped =");
        this_->mChoreographerStarted = false;
    }
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void AndroidDisplay::ChoreographerEnqueue()
{
    NS_ASSERT(mChoreographerEnabled);
    AChoreographer_postFrameCallback(AChoreographer_getInstance(), ChoreographerCallback, this);
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void AndroidDisplay::ChoreographerStart()
{
    if (mChoreographerEnabled)
    {
        if (IsReadyToRender())
        {
            if (!mChoreographerStarted)
            {
                NS_LOG_TRACE("= Choreographer started =");
                mChoreographerStarted = true;
                ChoreographerEnqueue();
            }
        }
    }
}
...
void AndroidDisplay::DispatchAppEvent(int eventId)
{
    switch (eventId)
    {
        ...
        case APP_CMD_TERM_WINDOW:
        {
            ...
            mHasWindow = false;
            mNativeHandleChanged(this, nullptr);
            break;
        }
    ...
    switch (AInputEvent_getType(event))
    {
        ...
        case AINPUT_EVENT_TYPE_MOTION:
        {
            ...
            switch (action & AMOTION_EVENT_ACTION_MASK)
            {
                ...
                {
                    int numPointers = (int)AMotionEvent_getPointerCount(event);

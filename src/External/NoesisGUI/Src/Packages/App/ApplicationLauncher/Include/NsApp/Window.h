...
public:
    ...
    void Init(Display* display, RenderContext* context, uint32_t samples, bool ppaa, bool lcd,
        bool emulateTouch, bool lazyRender);

    /// Destroys the active view
    void ResetView();

    /// Creates a new view in the given render context
    void RecreateView(RenderContext* context, bool emulateTouch);
    ...
    /// Gets or sets a value that indicates whether a window will automatically size itself to fit
    /// the size of its content
    //@{
    SizeToContent GetSizeToContent() const;
    void SetSizeToContent(SizeToContent value);
    //@}
...
public:
    ...
    static const Noesis::DependencyProperty* SizeToContentProperty;
...
private:
    ...
    void UpdateHistogram(uint16_t* data, float maxF, Noesis::MeshData* geometry) const;
    void CollectStats(double time);
    ...
    void OnDisplayScaleChanged(Display* displa, float scale);
    ...
    void SetClearColor(const Noesis::Color& color);
...
private:
    ...
    bool mLazyRender;
    bool mBackgroundIsSolid;
    Noesis::Color mClearColor;
    Noesis::Color mContextClearColor;
    ...
    Noesis::Ptr<Noesis::MeshData> mGeoHistCPU;
    Noesis::Ptr<Noesis::MeshData> mGeoHistGPU;
...
NS_DECLARE_REFLECTION_ENUM_EXPORT(NS_APP_APPLICATIONLAUNCHER_API, NoesisApp::SizeToContent)

...
namespace Noesis { class ResourceDictionary; }
...
public:
    ...
    /// Gets the render context of the application
    ...
    Noesis::ObjectWithNameScope FindNodeName(const char* name) const override;
    //@}
...
private:
    ...
    virtual bool GetLCDOverride() const;
    ...
    virtual bool GetEmulateTouchOverride() const;
    void EnsureResources() const;

    void OnDeviceLost();
    void CreateRenderContext(Display* display);

    void OnActivated(Display* display);
    void OnDeactivated(Display* display);
    void OnNativeHandleChanged(Display* display, void* window);
    void OnSaveState(Display* display);

...
void Application::Init(Display* display, const CommandLine& arguments)
{
    ...
    uint32_t samples = Max(GetSamplesOverride(), 1U);
    bool ppaa = GetPPAAOverride();
    bool lcd = GetLCDOverride();
    bool emulateTouch = GetEmulateTouchOverride();

    ApplicationLauncher* launcher = ((ApplicationLauncher*)Launcher::Current());
    bool lazyRender = launcher->GetLazyRenderOverride();

    mMainWindow->Init(display, mRenderContext, samples, ppaa, lcd, emulateTouch, lazyRender);
...
{
    NS_LOG_INFO("=== Device Lost ===");

    mRenderContext->DeviceLost() -= MakeDelegate(this, &Application::OnDeviceLost);

    mMainWindow->ResetView();
    mRenderContext->Shutdown();

    CreateRenderContext(mMainWindow->GetDisplay());
    mMainWindow->RecreateView(mRenderContext, GetEmulateTouchOverride());
}

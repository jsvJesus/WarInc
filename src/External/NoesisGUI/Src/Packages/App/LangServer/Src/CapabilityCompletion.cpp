...
////////////////////////////////////////////////////////////////////////////////////////////////////
static void PopulateNodeSnippets(SnippetSet& set, bool inDictionary)
{
    if (inDictionary)
    {
        set.Insert("controltmpl", SnippetData("Control Template with Key", SnippetData::Filter_ResourceDictionary, "<ControlTemplate x:Key=\"$1\" TargetType=\"{x:Type ${2:Button}}\">\n\t$3\n</ControlTemplate>$0", "ControlTemplate"));
        set.Insert("style", SnippetData("Style with Key", SnippetData::Filter_ResourceDictionary, "<Style x:Key=\"$1\" TargetType=\"{x:Type ${2:Button}}\">\n\t<Setter Property=\"$3\" Value=\"$4\" />$5\n</Style>$0", "Style"));
        set.Insert("style>tmpl", SnippetData("Style with Key > Template", SnippetData::Filter_ResourceDictionary, "<Style x:Key=\"$1\" TargetType=\"{x:Type ${2:Button}}\">\n\t<Setter Property=\"Template\">\n\t\t<Setter.Value>\n\t\t\t<ControlTemplate TargetType=\"{x:Type ${2:Button}}\">\n\t\t\t\t$3\n\t\t\t</ControlTemplate>\n\t\t</Setter.Value>\n\t</Setter>$4\n</Style>$0", "Style"));
        set.Insert("datatmpl", SnippetData("DataTemplate with Key", SnippetData::Filter_ResourceDictionary, "<DataTemplate x:Key=\"$1\" DataType=\"{x:Type $2}\">\n\t$3\n</DataTemplate>$0", "DataTemplate"));

        set.Insert("solidbrush", SnippetData("SolidColorBrush with Key", SnippetData::Filter_ResourceDictionary, "<SolidColorBrush x:Key=\"$1\" Color=\"$2\"/>$0", "SolidColorBrush"));
        set.Insert("linearbrush", SnippetData("LinearGradientBrush with Key", SnippetData::Filter_ResourceDictionary, "<LinearGradientBrush x:Key=\"$1\" StartPoint=\"0,0\" EndPoint=\"1,0\">\n\t<GradientStop Color=\"Gold\" Offset=\"0\" />\n\t<GradientStop Color=\"DarkOrange\" Offset=\"1\" />\n</LinearGradientBrush>$0", "LinearGradientBrush"));
        set.Insert("radialbrush", SnippetData("RadialGradientBrush with Key", SnippetData::Filter_ResourceDictionary, "<RadialGradientBrush x:Key=\"$1\">\n\t<GradientStop Color=\"Gold\" Offset=\"0\"/>\n\t<GradientStop Color=\"DarkOrange\" Offset=\"1\"/>\n</RadialGradientBrush>$0", "RadialGradientBrush"));
        set.Insert("imagebrush", SnippetData("ImageBrush with Key", SnippetData::Filter_ResourceDictionary, "<ImageBrush x:Key=\"$1\" ImageSource=\"$2\" Stretch=\"${3:UniformToFill}\"/>$0", "ImageBrush"));
    ...
    set.Insert("root-page", SnippetData("Page Root", SnippetData::Filter_Root, "<Page\n\txmlns=\"http://schemas.microsoft.com/winfx/2006/xaml/presentation\"\n\txmlns:x=\"http://schemas.microsoft.com/winfx/2006/xaml\">\n\t$0\n</Page>", "Page"));
    set.Insert("root-dict", SnippetData("ResourceDictionary Root", SnippetData::Filter_Root, "<ResourceDictionary\n\txmlns=\"http://schemas.microsoft.com/winfx/2006/xaml/presentation\"\n\txmlns:x=\"http://schemas.microsoft.com/winfx/2006/xaml\">\n\t$0\n</ResourceDictionary>", "ResourceDictionary"));
        ...
        if (parts[partIndex].HasFlag(NoesisApp::PartFlags_HasResources))
        {
            ...
            else
            {
                ...
                for (; resNodePartIndex < (int32_t)parts.Size(); resNodePartIndex++)
                {
                    if (parts[resNodePartIndex].partKind == NoesisApp::XamlPartKind_StartTagBegin
                        && !parts[resNodePartIndex].HasErrorFlag(NoesisApp::ErrorFlags_IdError)
            ...
            for (int32_t i = resNodePartIndex + 1; i < (int32_t)parts.Size(); i++)
            {
                    ...
                    && checkPart.HasFlag(NoesisApp::PartFlags_IsResource))
                {
                    ...
                    if (owner.HasErrorFlag(NoesisApp::ErrorFlags_IdError))
                    {
                        continue;
                    }
...
static const Noesis::TypeClass* GetPropertyTypeClass(const Noesis::TypeClass* targetTypeClass,
    ...
    int32_t& targetPartIndex, Noesis::BaseComponent* resource)
{
    const Noesis::TypeProperty* typeProperty = nullptr;
    const Noesis::DependencyProperty* depProperty = nullptr;
    ...
    bool addEqualsQuotations, bool isRoot, Noesis::BaseString& responseBuffer)
{
    ...
    for (const auto& entry : items)
    {
        ...
        Noesis::String sortText(Noesis::String::VarArgs(), "%02i_%s", entry.value.itemKind,
            entry.key.Str());
    ...
    const TextPosition& position, Noesis::BaseString& responseBuffer)
{
    ...
    {
        SnippetSet set;
        PopulateNodeSnippets(set, false);
    ...
    if (part.partKind == XamlPartKind_StartTagBegin
        && ownerPart.partKind == XamlPartKind_StartTagBegin
        && ownerPart.HasFlag(PartFlags_Extension))
    {
        MessageWriter::CreateErrorResponse(bodyId, LSPErrorCodes::RequestCancelled,
            "RequestCancelled: Extensions as extension parameters are not supported.", responseBuffer);
        return;
    }
    ...
    if (isContent && position.character > part.startCharacterIndex)
    {
        ...
        || part.partKind == XamlPartKind_TagContent)
    {
        ...
        else if (part.partKind == XamlPartKind_TagContent)
        {
            ...
            {
                inDictionary = true;
                PopulateNodeSnippets(set, true);
                for (const auto& entry : set)
                {
                    if (entry.value.filter == SnippetData::Filter_ResourceDictionary)
                    {
                        items.Insert(entry.key.Str(), CompletionItemData(ItemKindSnippet,
                            entry.value.detail.Str(), entry.value));
                    }

    ...
    Noesis::BaseString& responseBuffer)
{
    ...
    if (!uri.IsValid())
    {
        NS_LOG_INFO("Method '%s', Uri '%s' is not valid.\n", method, uri.Str());
        return true;
    }
    ...
    Noesis::BaseString& responseBuffer)
{
    if (!body.IsValid())
    {
        NS_LOG_INFO("The message does not contain a valid body");
        return;
    }
    ...
    if (Noesis::StrEquals(method, "xaml/autoInsert"))
    {
        ...
        {
            MessageWriter::CreateErrorResponse(body.GetInt32Value("id"),
                LSPErrorCodes::RequestFailed, "RequestCancelled: Document is not open.",
                responseBuffer);
            return;
        }

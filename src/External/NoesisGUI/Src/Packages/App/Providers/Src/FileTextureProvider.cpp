...
TextureInfo FileTextureProvider::GetTextureInfo(const Uri& uri)
{
    ...
    if (base != nullptr)
    {
        ...
        int x, y;
        if (stbi_info_from_memory(base, file->GetLength(), &x, &y, nullptr))
        {
            info.width = x;
            info.height = y;
        }
        ...
        {
            NS_ERROR("SEH Exception reading mapped file at %p", base);
        }
      #endif
    }
    else
    {
        ...
        if (stbi_info_from_callbacks(&callbacks, file, &x, &y, nullptr))
        {
            info.width = x;
            info.height = y;
        }

...
public: // internal
	...
	void SortNavMeshAccordingToGuidCompound(CellBox& currentUpdateCellBox);
...
public:
	...
	KyResult CheckGenerationParameters(const NavMeshElementBlob::GenParameters& navMeshGenParameters);
	...
	bool IsCompatibleWith(const NavMeshElementBlob& navMeshElementBlob) const { return IsCompatibleWith(navMeshElementBlob.m_genParameters); }
	bool IsCompatibleWith(const NavMeshElementBlob::GenParameters& navMeshGenParameters) const;

public: // internal
	// Used by the generationd
	NavMeshElement* SingleInsertion(const NavMeshElementBlob* navMeshElementBlob);
...
KY_INLINE void NavMeshElementManager::SortNavMeshAccordingToGuidCompound(CellBox& currentUpdateCellBox)
{
	m_guidCompoundSorter.Sort(m_activatedGuids, currentUpdateCellBox);
}

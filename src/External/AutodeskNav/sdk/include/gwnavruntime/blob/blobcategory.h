...
public:
	...
	template<class T> void AddBlobType() { m_descriptors.PushBack(*KY_NEW_ID(MemStat_BaseSystem) BlobTypeDescriptor<T>); }

	void Reserve(KyUInt32 count) { return m_descriptors.Reserve(count); }

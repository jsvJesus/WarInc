...
enum PathEventType
{
	...
	PathEventType_OutsideConnection,      ///< The PathEvent is at a transition between the NavMesh and a PathEdge going/coming to/from outside.
	PathEventType_OutsideNavMeshPathNode, ///< The PathEvent is outside the NavMesh.
	...
	/// PathValidityInterval bound specific: bound hit a border or a traversable
	///  navTag boundary during validation.
	PathEventType_OnNavMeshAfterCollistion
};
...
public:
	...
	void SetCheckPointStatus(CheckPointStatus checkPointStatus) { m_checkPointStatus = checkPointStatus; }
	bool IsACheckPoint() const { return m_checkPointStatus == CheckPointStatus_EventIsACheckPoint; }
	bool IsOnUpperBound() const { return m_eventStatusInList == PathEventStatus_UpperBound || m_eventStatusInList == PathEventStatus_TemporaryUpperBound; }
	bool IsOnLowerBound() const { return m_eventStatusInList == PathEventStatus_LowerBound; }
...
public:
	...
	/// \pre    eventIdx is different from 0, the index of the first event.
	const NavTag* GetNavTagOnIntervalIdxBeforeEvent(KyUInt32 eventIdx) const;

	/// Returns a pointer to the NavTag lying under the interval starting at the event of index eventIdx.
	/// \pre    eventIdx is different from GetPathEventIntervalCount() - 1, the index of the last event.
	const NavTag* GetNavTagOnIntervalIdxAfterEvent(KyUInt32 eventIdx) const;
	...
	Ptr<Path> m_path;
	KyArray<PathEventInterval, MemStat_Bot> m_pathEventIntervalArray; // the fist interval is a dummy one used to store the first event.
};


KY_INLINE PathEventList::PathEventList() : m_path(KY_NULL) {}
KY_INLINE PathEventList::~PathEventList() { Clear(); m_path = KY_NULL; }
...
{
	KY_LOG_ERROR_IF(eventIdx >= GetPathEventCount(), ("Input index is not a valid event index"));
	KY_LOG_ERROR_IF(eventIdx == GetPathEventCount() - 1, ("You cannot retrieve the index of the interval before the first event. It will return unused index"));
	return eventIdx;
}

KY_INLINE const NavTag* PathEventList::GetNavTagOnIntervalIdxBeforeEvent(KyUInt32 eventIdx) const
{
	return GetNavTagOfEventInterval(GetIntervalIdxBeforeEvent(eventIdx));
}
KY_INLINE const NavTag* PathEventList::GetNavTagOnIntervalIdxAfterEvent(KyUInt32 eventIdx) const
{

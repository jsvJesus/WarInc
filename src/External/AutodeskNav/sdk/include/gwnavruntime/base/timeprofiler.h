...
/// Small utility class atop Timer to profile easily. For profiling, use TimeProfiler or ScopedProfiler to directly use Kaim::Timer.
/// Note: TimeProfiler is not used in the ScopedProfiler implementation because it does not call Timer::GetProfileTicks() when time = KY_NULL
...
#define KY_BEGIN_PROFILE_SUM_SECONDS(s) { Kaim::ScopedProfilerSumSeconds scopedProfiler(s);
#define KY_BEGIN_PROFILE_SUM_MS(ms)     { Kaim::ScopedProfilerSumMs      scopedProfiler(ms);
#define KY_BEGIN_PROFILE_SUM_MKS(mks)   { Kaim::ScopedProfilerSumMks     scopedProfiler(mks);

#define KY_END_PROFILE }


/// Small utility class atop Kaim::Timer to profile easily. For profiling, use
/// ScopedProfiler or Profiler rather than Timer.
class Profiler
{
	KY_DEFINE_NEW_DELETE_OPERATORS(Stat_Default_Mem)

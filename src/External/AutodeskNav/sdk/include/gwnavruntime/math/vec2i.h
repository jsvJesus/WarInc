...
public:
	...
	KY_INLINE bool operator==(const Vec2i& v) const { return x == v.x && y == v.y; }
	...
	/// Returns true if the size of this vector along both the X and Y axes is more than that of v. 
	KY_INLINE bool operator>(const Vec2i& v) const
	{
	...
	/// Set theneighbor vector coordinates to identify the next grid cell in the direction of the specified CardinalDir. 

...
public:
	...
	/// Returns true if this edge can be traversed according the CanTraverse function in theNavTagTraversePredicate.
	/// \param predicate    A pointer to a predicate that has this function implemented : <code>bool CanTraverse(const NavTag& navTag)</code>.
	///                      This is typically the same predicate that you passed to the a query or to a PathFollower.
	/// \pre    This object must be valid. Its validity is not checked.
	...
	/// \pre    This object must be valid. Its validity is not checked.
	template <class NavTagTraversePredicate>
	bool IsHalfEdgeCrossable(NavHalfEdgePtr& resultPtr, NavTagTraversePredicate* predicate) const;
	...
	///          another triangle with a traversable NavTag. That means we must have <code>IsHalfEdgeCrossable<NavTagTraversePredicate>() == false</code>.
	/// \post    The returned edge is not paired with another triangle with a traversable NavTag.
	template<class NavTagTraversePredicate>
	NavHalfEdgePtr GetNextNavHalfEdgePtrAlongBorder(NavTagTraversePredicate* predicate) const;

	/// Returns the previous edge along the border of the NavMesh according to NavTagTraversePredicate.
	/// \param predicate    A pointer to a predicate that has this function implemented : <code>bool CanTraverse(const NavTag& navTag)</code>.
	///                      This is typically the same predicate that you passed to the a query or to a PathFollower.
	/// \pre    This object must be valid. Its validity is not checked.
	/// \pre    This object must be on the border of an area of the NavMesh according to NavTagTraversePredicate. It may not be paired with
	...
	/// \post    The returned edge is not paired with another triangle with a traversable NavTag.
	template<class NavTagTraversePredicate>
	NavHalfEdgePtr GetPrevNavHalfEdgePtrAlongBorder(NavTagTraversePredicate* predicate) const;
...
template<class NavTagTraversePredicate>
KY_INLINE bool NavHalfEdgePtr::IsHalfEdgeCrossable(NavTagTraversePredicate* predicate) const
{
	return GetRawPtr().IsHalfEdgeCrossable<NavTagTraversePredicate>(predicate);
}

template<class NavTagTraversePredicate>
KY_INLINE NavHalfEdgePtr NavHalfEdgePtr::GetNextNavHalfEdgePtrAlongBorder(NavTagTraversePredicate* predicate) const
{
	return NavHalfEdgePtr(GetRawPtr().GetNextNavHalfEdgeRawPtrAlongBorder<NavTagTraversePredicate>(predicate));
}

template<class NavTagTraversePredicate>
KY_INLINE NavHalfEdgePtr NavHalfEdgePtr::GetPrevNavHalfEdgePtrAlongBorder(NavTagTraversePredicate* predicate) const
{
	return NavHalfEdgePtr(GetRawPtr().GetPrevNavHalfEdgeRawPtrAlongBorder<NavTagTraversePredicate>(predicate));
}
...
template<class NavTagTraversePredicate>
inline bool NavHalfEdgePtr::IsHalfEdgeCrossable(NavHalfEdgePtr& resultPtr, NavTagTraversePredicate* predicate) const
{
	NavHalfEdgeRawPtr resultRawPtr;
	bool storeResult = GetRawPtr().IsHalfEdgeCrossable<NavTagTraversePredicate>(resultRawPtr, predicate);
	resultPtr = NavHalfEdgePtr(resultRawPtr);
	return storeResult;
}

...
public:
	DynamicNavCell(MemoryHeap* heap, const CellDesc& cellDesc, bool isOnOverlap = false) 
		: m_cellDesc(cellDesc), 
		m_floors(heap),
		m_isOnOverlap(isOnOverlap)
	{}
	...
	bool IsEmpty()
	{
		...
		for (KyUInt32 i = 0; i < m_floors.GetCount(); ++i)
		{
			...
			if (floor.m_edges.IsEmpty() && floor.m_triangles.IsEmpty())
			{
				continue;
			}
			return false;

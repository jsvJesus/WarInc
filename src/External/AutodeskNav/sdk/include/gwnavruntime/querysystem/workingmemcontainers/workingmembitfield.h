...
public:
	...
	bool IsBitSet(KyUInt32 bitIndex);

	void UnsetAllBits();
	void SetAllBits();


	KyResult SetMinimumBitCount(KyUInt32 bitCount) /* \pre IsInitialized() == true */
	{
		const KyUInt32 wordCount = BitFieldBase::GetWordsCount(bitCount);
...
KY_INLINE void WorkingMemBitField::SetBit(KyUInt32 bitIndex)   { BitFieldBase::SetBit(GetWordsBuffer(), bitIndex);          }
KY_INLINE void WorkingMemBitField::UnsetBit(KyUInt32 bitIndex) { BitFieldBase::UnsetBit(GetWordsBuffer(), bitIndex);        }
KY_INLINE bool WorkingMemBitField::IsBitSet(KyUInt32 bitIndex) { return BitFieldBase::IsBitSet(GetWordsBuffer(), bitIndex); }

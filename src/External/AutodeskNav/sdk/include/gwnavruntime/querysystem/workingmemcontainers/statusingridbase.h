...
public:
	...
	struct IndexedStatus
	{
		...
		KY_INLINE bool IsNodeOpen(KyUInt32 triangleIdx) const { return BitFieldBase::IsBitSet(GetBufferOfStatus(), triangleIdx); }
		KY_INLINE void OpenNode(KyUInt32 triangleIdx) const { BitFieldBase::SetBit(GetBufferOfStatus(), triangleIdx); }
...
{
	return (StatusInGridBase::StatusWord*)AllocateInBufferAndMemset(sizeof(StatusInGridBase::StatusWord), BitFieldBase::GetWordsCount(numberOfNodes), 0);
}

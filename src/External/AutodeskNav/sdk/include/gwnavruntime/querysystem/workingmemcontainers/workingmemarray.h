...
public:
	...
	const T& operator[](KyUInt32 idx) const;        /* \pre IsInitialized() == true && idx < GetCount() */
	T& operator[](KyUInt32 idx);                    /* \pre IsInitialized() == true && idx < GetCount() */

	KyResult TryToResize(); /* \pre IsInitialized() == true */

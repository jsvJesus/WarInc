...
enum LogConstants
{
    ...
    Log_MessageLine  = LogChannel_General | LogMessage_TextLine,
    ...
    Log_DebugMessageLine = LogChannel_General | LogMessage_TextLine,
...
public: 
    ...
    inline void LogMessageLine(const char* fmt, ...) KY_LOG_VAARG_ATTRIBUTE(2,3)
    {
        va_list argList; va_start(argList, fmt);
        LogMessageVarg(Log_MessageLine, fmt, argList);
        va_end(argList);
    }
...
// class used internally to hold message information in logging macros
class LogMessageHelper
{
public:
    LogMessageHelper(const char* file, UInt32 line) 
        : m_id(0)
        , m_file(file)
        , m_line(line)
    {}

    LogMessageHelper(LogMessageId id, const char* file, UInt32 line) 
        : m_id(id)
        , m_file(file)
        , m_line(line)
    {}

    const LogMessageHelper& MakeMessage(LogMessageId id, const char* fmt, ...) KY_LOG_VAARG_ATTRIBUTE(3,4);
    const LogMessageHelper& MakeMessage(const char* fmt, ...) KY_LOG_VAARG_ATTRIBUTE(2,3);

private:
    bool MustAddFilenameAndLine();
...
public:
    ...
    const char* m_file;
    UInt32 m_line;
    char m_message[Log::MaxLogBufferMessageSize];
};

...
public:
	...
	KyResult InsertOrUpdateInBinHeapTraversal(AStarNode& neighbourNode, AStarNodeIndex neighbourNodeIndex);
...
KY_INLINE KyResult AStarTraversalContext::InsertOrUpdateInBinHeapTraversal(AStarNode& neighbourNode, AStarNodeIndex neighbourNodeIndex)
{
	if (neighbourNode.m_indexInBinaryHeap < IndexInBinHeap_Removed)
	{
		// (the node is open, already present in the binary heap), we compute its new position in the BinaryHeap
		m_traversalBinHeap.UpdateAt(neighbourNode.m_indexInBinaryHeap);
	}
	else
	{
		...
		m_traversalBinHeap.Insert(neighbourNodeIndex);
	}

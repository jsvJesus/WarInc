...
template <class SpatializedPointCollectorFilter>
KY_INLINE void SpatializedPointCollectorInAABBQuery<SpatializedPointCollectorFilter>::Initialize(Database* database, const Vec3f& startPos, const Box3f& aABB)
{
	BaseSpatializedPointCollectorInAABBQuery::Initialize(database, startPos, aABB);
}
...
void SpatializedPointCollectorInAABBQuery<SpatializedPointCollectorFilter>::PerformQueryWithInputCoordPos(WorkingMemory* workingMemory)
{
	...
	{ // Scope to be sure we release the working memory after the result has been copied into m_queryDynamicOutput 
		...
		scopeAutoSaveDynamicOutput.SetSpatializedPoints(&workingMemory->m_collectorContext->m_resultCollection);

		RunCollectorTraversal(workingMemory->m_collectorContext);
	}

	workingMemory->m_collectorContext->ReleaseWorkingMemory();
}
...
inline KyResult SpatializedPointCollectorInAABBQuery<SpatializedPointCollectorFilter>::ProcessNavFloorNode(SpatializedPointCollectorContext* traversalContext, const NavFloorRawPtr& navFloorRawPtr)
{
	...
	KY_FORWARD_ERROR_NO_LOG(TraverseNeighbourNavFloors(traversalContext, navFloorRawPtr));
	KY_FORWARD_ERROR_NO_LOG(TraverseNeighbourNavGraphEdges(traversalContext, navFloorRawPtr));
	return KY_SUCCESS;
}
...
inline KyResult SpatializedPointCollectorInAABBQuery<SpatializedPointCollectorFilter>::ProcessNavGraphEdgeNode(SpatializedPointCollectorContext* traversalContext, const NavGraphEdgeRawPtr& navGraphEdgeRawPtr)
{
	...
	KY_FORWARD_ERROR_NO_LOG(TraverseNeighbourNavFloors(traversalContext, navGraphEdgeRawPtr));
	KY_FORWARD_ERROR_NO_LOG(TraverseNeighbourNavGraphEdges(traversalContext, navGraphEdgeRawPtr));
	return KY_SUCCESS;
}

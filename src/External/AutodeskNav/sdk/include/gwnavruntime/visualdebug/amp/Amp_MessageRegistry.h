...
public:
    ...
    {
        BaseMessageTypeDescriptor* myDescriptor = GetMessageTypeDescriptor(TMessage::GetTypeName());
        if (myDescriptor == NULL)
        {
            ...
            StringToMsgType.Add(TMessage::GetTypeName(), localMsgType);
            Descriptors.PushBack(descriptor);
        }
        else
        {
            KY_DEBUG_WARNINGN_IF(myDescriptor->MessageHandler != NULL, ("Handler of descriptor %u, %s-%u is overridden", static_cast<unsigned int>(myDescriptor->GetMessageType()), myDescriptor->GetMessageTypeName().ToCStr(), static_cast<unsigned int>(myDescriptor->GetMessageLatestVersion())));
    ...
    {
        const UInt32* pMsgType = StringToMsgType.Get(messageTypeName);
        BaseMessageTypeDescriptor* descriptor = (pMsgType ? Descriptors[*pMsgType] : NULL);
        while (descriptor)
        {
            ...
            {
                pMsgType = StringToMsgType.GetAlt(messageTypeName);
                descriptor = (pMsgType ? Descriptors[*pMsgType] : NULL);
            }
        }
    ...
    UInt32 GetMessageType(const String& messageTypeName) const
    {
        ...
        return (descriptor ? descriptor->GetMessageType() : InvalidMsgType);
    }
    ...
    UInt32 GetLocalMessageType(UInt32 peerMsgType) const
    {
        if (MappingEnabled)
        {
            ...
            return InvalidMsgType; //in that case there is a problem, the type are not properly mapped
        }

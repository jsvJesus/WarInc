...
class NavGraph
{
	...
	NavGraph(const NavGraphBlob* navGraphBlob, Database* database);
public:
...
public:
	...
	NavTag* m_navTags; // the navTag presents in the NavFloorBlob are copied in the navGraphBlob.
...
public: // Lifetime - separate implementation due to Create function
	void AddRef();
	void Release();
	// Debug method only.
...
{
	DebugCheckIsStillLoaded();
	return m_edgeDatas[m_vertexDatas[navGraphEdgeSmartIdx.GetStartVertexIdx()].m_startIdxInEdgeDataArray + navGraphEdgeSmartIdx.GetNeighbourVertexIdx()];
}
...
KY_INLINE void NavGraph::AddRef()  { RefCount++; }
KY_INLINE void NavGraph::Release() 
{

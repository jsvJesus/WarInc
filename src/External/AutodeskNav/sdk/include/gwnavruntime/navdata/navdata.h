...
public:
	...
	/// Returns the Name passed to Kaim::GeneratorSectorConfig. Used for VisualDebug to access the NavData and ClientInput files.
	const String& GetSectorName() const;
	/// Returns the relative output directory provided to the generator when this NavData was built
	...
	/// Can be used in Generation Post Process to save a graph
	...
	/// Indicates whether the specifiedNavData has been effectively added to the Database and not 
	///  effectively removed yet from the Database. That means that, through the Database, it will be used for
	/// Query processing andBot PathFollowing, etc...
...
KY_INLINE NavData::NavData(Database* database) : m_database(KY_NULL), m_databaseStatus(NotReferencedInAnyDatabase), m_visualDebugStatus(VisualDebugStatus_NothingToSend)
{
	Init(database);
}

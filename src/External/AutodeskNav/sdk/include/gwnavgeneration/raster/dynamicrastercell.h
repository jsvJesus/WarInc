...
public:
	DynamicRasterCell(
		const GeneratorParameters& config, const CellDesc& cellDesc, const KyArrayPOD<const PdgInputCell*>* inputCells,
	...
	{
		KyFloat32 rasterPrecision = config.GetNormalizedRasterPrecision();
	...
	void Setup(const GeneratorParameters& config, const CellDesc& cellDesc,
			   const KyArrayPOD<const PdgInputCell*>* inputCells, KyFloat32 rasterPrecision, const PixelBox& pixelBox);
	...
	*/
	...
	KyResult WriteFiles(GeneratorSystem* system) const;

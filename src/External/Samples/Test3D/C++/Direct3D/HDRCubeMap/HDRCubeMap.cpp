...
#define IDC_PPSTEREOSCOPY     12
#define IDC_SLIDERSTEREO      13
#define IDC_RELOADSHADER      14
...
void RenderScene( IDirect3DDevice9* pd3dDevice, const D3DXMATRIX* pmView, const D3DXMATRIX* pmProj,
                  CTechniqueGroup* pTechGroup, bool bRenderEnvMappedMesh, double fTime, bool Depth );
...
void InitApp()
{
	...
	g_HUD.AddButton( IDC_RELOADSHADER, L"Reload Shader (F5)", 35, iY += 24, 125, 22, VK_F5 );
	...
	g_HUD.AddCheckBox( IDC_PPSTEREOSCOPY, L"Use PP Stereoscopy", 35, iY += 24, 130, 22, false, L'F' );
	g_HUD.AddSlider( IDC_SLIDERSTEREO, 35, iY += 24, 125, 22, 0, 100 );	
...
void UpdateUiWithChanges()
{
	...
	if( CDXUTSlider* pSlider = g_HUD.GetSlider( IDC_SLIDERSTEREO ) )
	{
		pSlider->SetValue( int( g_fStereoStrength * 100.0f ) );
	}
	...
	pCheck = g_HUD.GetCheckBox( IDC_PPSTEREOSCOPY );
	if( pCheck )
		pCheck->SetChecked( g_bUsePPStereoscopy );
...
HRESULT ReloadEffectzzZz( IDirect3DDevice9* pd3dDevice )
{
    ...
    HRESULT hrr = D3DXCreateEffectFromFile( pd3dDevice, str, NULL, NULL, dwShaderFlags,
											NULL, &g_pEffect, &buff );

	if( buff && hrr != S_OK )
	{
		const char* text = (const char*)buff->GetBufferPointer();
		const wchar_t* wtext = (const wchar_t*)buff->GetBufferPointer();
                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	Start3D( pBackBufferSurfaceDesc->Width, pBackBufferSurfaceDesc->Height );
    ...
    g_Camera.SetProjParams( D3DX_PI / 4, fAspectRatio, 0.25f, 16000.0f );
    g_Camera.SetWindow( pBackBufferSurfaceDesc->Width, pBackBufferSurfaceDesc->Height );

    g_HUD.SetLocation( pBackBufferSurfaceDesc->Width - 170, 0 );
    g_HUD.SetSize( 170, 170 );
    int iY = pBackBufferSurfaceDesc->Height - 220;
    CDXUTControl* pControl;
    if( ( pControl = g_HUD.GetControl( IDC_CHECKHDR ) ) != NULL )
        pControl->SetLocation( 35, iY );
	if( ( pControl = g_HUD.GetControl( IDC_PPSTEREOSCOPY ) ) != NULL )
		pControl->SetLocation( 35, iY += 24 );
	if( ( pControl = g_HUD.GetControl( IDC_SLIDERSTEREO ) ) != NULL )
		pControl->SetLocation( 35, iY += 24 );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
		if( Is3DOn() )
		{
			if( g_bUsePPStereoscopy )
			{
				V( pd3dDevice->SetRenderTarget( 0, GetDepth() ) );

				// Clear the render buffers
				V( pd3dDevice->Clear(	0L, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER,
										0x000000ff, 1.0f, 0L ) );

				RenderScene( pd3dDevice, g_Camera.GetViewMatrix(), g_Camera.GetProjMatrix(), g_pTech, true, fTime, true );

				V( pd3dDevice->SetRenderTarget( 0, GetCenterTarget() ) );

				// Clear the render buffers
				V( pd3dDevice->Clear(	0L, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER,
										0x000000ff, 1.0f, 0L ) );

				RenderScene( pd3dDevice, g_Camera.GetViewMatrix(), g_Camera.GetProjMatrix(), g_pTech, true, fTime, false );
			...
			else
			{
				...
				CalcStereoViewProj( &g_Camera, &leftView, &leftProj, &rightView, &rightProj, g_fStereoStrength );

				V( pd3dDevice->SetRenderTarget( 0, GetLeftTarget() ) );

				// Clear the render buffers
				V( pd3dDevice->Clear(	0L, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER,
										0x000000ff, 1.0f, 0L ) );

				RenderScene( pd3dDevice, &leftView, &leftProj, g_pTech, true, fTime, false );


				V( pd3dDevice->SetRenderTarget( 0, GetRightTarget() ) );

				// Clear the render buffers
				V( pd3dDevice->Clear(	0L, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER,
										0x000000ff, 1.0f, 0L ) );

				RenderScene( pd3dDevice, &rightView, &rightProj, g_pTech, true, fTime, false );
			}
		}
		else
		{
		...
		if( Is3DOn() )
		{
			Trick3D( g_pEffect, &g_Camera, false, g_bUsePPStereoscopy, g_fStereoStrength );
		}
...
void RenderSceneIntoCubeMap( IDirect3DDevice9* pd3dDevice, double fTime )
{
    ...
    for( int nCube = 0; nCube < g_nNumCubes; ++nCube )
        for( int nFace = 0; nFace < 6; ++nFace )
        {
            ...
            if( SUCCEEDED( pd3dDevice->BeginScene() ) )
            {
                RenderScene( pd3dDevice, &mView, &mProj, &g_pTech[nCube], false, fTime, false );
                  ...
                  CTechniqueGroup* pTechGroup, bool bRenderEnvMappedMesh, double fTime, bool Depth )
{
    ...
    if( bRenderEnvMappedMesh )
    {
		if( Depth )
		{
			V( g_pEffect->SetTechnique( g_pEffect->GetTechniqueByName( "RenderVictoriousDepth" ) ) );
		}
		else
		{
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		case IDC_RELOADSHADER:
			ReloadEffectzzZz( DXUTGetD3D9Device() );
			break ;
        case IDC_CHECKHDR:
		...
		case IDC_PPSTEREOSCOPY:
			g_bUsePPStereoscopy = ( ( CDXUTCheckBox* )pControl )->GetChecked();
			break;
        case IDC_CHANGEMESH:

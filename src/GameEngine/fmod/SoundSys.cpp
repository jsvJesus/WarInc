...
void CSoundSystem::Update(const r3dPoint3D &pos, const r3dPoint3D &dir, const r3dPoint3D& up)
{
	...
	left.Normalize();
	r3dPoint3D correctUp = dir.Cross(left);
	correctUp.Normalize();
	result = FMODEventSystem->set3DListenerAttributes(0, (const FMOD_VECTOR*)&pos, 0, (const FMOD_VECTOR*)&dir, (const FMOD_VECTOR*)&correctUp);
	SND_ERR_CHK(result);
...
void AppendParentRecursively(std::string &path, FMOD::EventGroup *eg);
void AppendParentProject(std::string &path, FMOD::EventProject *ep);

void AppendParentRecursively(std::string &path, FMOD::Event *e)
{
	char *buf = 0;
	e->getInfo(0, &buf, 0);
	if (buf)

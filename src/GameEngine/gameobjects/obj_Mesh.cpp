...
void CacheMeshFunc(void *params)
{
	...
	pMesh->Load(fname, p->use_default_material, !p->allow_async );
...
//--------------------------------------------------------------------------------------------------------
r3dMesh *r3dGOBAddMesh(const char* fname, bool addToLibrary, bool use_default_material, bool allow_async, bool player_mesh )
{
	char szFixedName[MAX_PATH];	
	FixFileName(fname, szFixedName);
...
int	MeshGameObject::ChoseMeshLOD( float distSq, r3dMesh* (&TargetLOD)[ NUM_LODS ] )
{
	...
	float currentMeasure = CalculateLodMeasure(radius, sqrtf(distSq));
...
MeshGameObject::GetDestructionKey() const
{
	...
	if( const char* start = strstr( tempFName, COMMON_DEPOT_PREFIX ) )
	{
		...
		return g_DamageLib->ComposeKey( Dir, std::string( File ) + Ext );
	}
...
static bool FillDestructionMeshName( char (&oName) [ MAX_PATH ], DamageLibEntry* dlibEntry )
{
	if( dlibEntry && dlibEntry->MeshName[ 0 ] )
	{
...
void MeshGameObject::Draw(const r3dCamera& Cam, eRenderStageID DrawState)
{
	...
	if( DrawState == rsFillGBuffer || IsShadowPath )
	{
		...
		{
			MeshLOD[mainCameraLod]->SetVSConsts(mTransform);
			if( IsShadowPath )
...
struct DepthMeshRenderable : Renderable
{
	...
	static void Draw( Renderable* RThis, const r3dCamera& Cam )
	{
		...
		This->Mesh->SetVSConsts( This->Parent->GetTransformMatrix() );
								...
								const char* szFileName, bool LoadCollision )
{
	...
	TargetLODs[0] = r3dGOBAddMesh( szFileName, true, use_default_mat );
	...
	{
		TargetLODs[1] = r3dGOBAddMesh(TempLodName);
	}
	sprintf(TempLodName, "%s_LOD2.sco", TempStr1);
	if(r3d_access(TempLodName, 0)==0)
	{
		TargetLODs[2] = r3dGOBAddMesh(TempLodName);
	}
	sprintf(TempLodName, "%s_LOD3.sco", TempStr1);
	if(r3d_access(TempLodName, 0)==0)
	{
		TargetLODs[3] = r3dGOBAddMesh(TempLodName);
	}

...
namespace
{
	...
	bool FillGrassChunk( GrassChunk& oChunk, const GrassPatchGenSettings& patchSettings, int Ordinal )
	{
		...
		if( !mesh->Load( grassMeshPath.c_str() ) )
		{
		...
		oChunk.IndexBuffer = new r3dIndexBuffer( numInstances * mesh->NumIndices, false, 2, GrassBufferMem );
		...
		for( uint32_t i = 0, e = settings.NumVariations; i < e; i ++ )
		{
			...
			if(g_bEditMode)
				readable = true;
#endif
			oChunk.VertexBuffs[ i ] = new r3dVertexBuffer( numVertices, sizeof( GrassVertex ), 0, false, readable );
		}
...
GrassGen::FillTypes( string_vec& oVec )
{
	...
	for( uint32_t i = 0, e = mPatchSettings.Count(); i < e; i ++ )
	{
		oVec.push_back( std::string( mPatchSettings[ i ].Name.c_str() ) );
	}
}

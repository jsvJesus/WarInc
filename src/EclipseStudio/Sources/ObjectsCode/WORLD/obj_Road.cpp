...
//		const pnt_s& cp2 = orgPoints_[seg+1];
	...
	// if you edit that copy to RoadEditor.cpp
...
void obj_Road::recalcNormals()
{
	...
	{
		r3dVector normal = CalcFaceNormal(mesh_->VertexPositions[mesh_->Indices[face*3+0]], mesh_->VertexPositions[mesh_->Indices[face*3+1]], mesh_->VertexPositions[mesh_->Indices[face*3+2]]); 
		for(int i=0; i<3; i++) {
...
{
	char * data = new char[sizeof(HeaderType) + R3D_MAX_MATERIAL_NAME];
...
void obj_Road::WriteSerializedData(pugi::xml_node& node)
{
	...
	HeaderType* pHdr = ( HeaderType* ) new char[ sizeof( HeaderType ) + R3D_MAX_MATERIAL_NAME ];
	HeaderType& hdr = *pHdr;
...
int obj_Road::edGetClosestPoint(const r3dPoint3D& pnt, int* isNormal)
{
	for(unsigned int i=0; i<orgPoints_.size(); i++) {
		const pnt_s& cp = orgPoints_[i];
		r3dPoint3D d1   = cp.p - pnt;
		float      len1 = sqrtf(d1.x*d1.x + d1.z*d1.z);

		r3dPoint3D d2   = (cp.p + cp.n) - pnt;
		float      len2 = sqrtf(d2.x*d2.x + d2.z*d2.z);

		if(len1 < _road_PointSelDist || len2 < _road_PointSelDist) {
			if(isNormal) *isNormal = (len1 < len2) ? 0 : 1;
			return i;
		}
	}
...
void obj_Road::SetMaterial( const char* szName )
{
	if ( mesh_ )
	{
		...
		r3dMaterial* pMat = r3dMaterialLibrary::RequestMaterialByMesh( szName, BaseMaterialDir, false );
		if ( pMat )
		{
			batch.Mat = pMat;

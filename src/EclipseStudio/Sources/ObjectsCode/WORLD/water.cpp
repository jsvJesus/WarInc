...
void WaterBase::RenderBegin(const r3dCamera& Cam, float waterLevel, bool followTerrain)
{
	...
	char* q[] = { "_L", "_N", "_H" };
	strcat(sVS, q[r_water_quality->GetInt()-1]);
	...
	*(D3DXMATRIX*)( psConsts + 3 ) = RefractViewProjTexXf ;
	...
	D3DXVECTOR4 fShallowColor_Depth = D3DXVECTOR4 ( _shallowColor.R * ic, _shallowColor.G * ic, _shallowColor.B * ic, 0.0f );
	...
	if(r3dRenderer->CameraPosition.y <= waterLevel)
	{
		float dc = waterLevel - r3dRenderer->CameraPosition.y;
...
void WaterBase::CreateRefractionBuffer()
{
	...
	curQualitySettings = r_water_quality->GetInt();

...
namespace
{
	...
	{
		return r_texture_quality->GetInt() < 2 ? 2 : 1 ;
	}
...
{
	mLastLoadDecoQL = r_decoration_quality->GetInt() ;
...
DecalChief::SaveLevel( const r3dString& levelPath ) const
{
	//------------------------------------------------------------------------
	pugi::xml_document xmlLibFile;
	xmlLibFile.append_child( pugi::node_comment ).set_value("Decal Settings File");
	pugi::xml_node xmlItems = xmlLibFile.append_child();
	xmlItems.set_name( "decal_settings" );

	xmlItems.append_attribute( "lib" ) = GLOBAL_DECAL_LIB;

	xmlLibFile.save_file( GetFullXMLPath( levelPath ).c_str() );
...
DecalChief::LoadTextures( bool staticOnly )
{
	...
	typedef std::set< DecalType* > DecalTypePtrSet ;
...
DecalChief::ReloadTextures()
{
	...
	typedef std::map< std::string, r3dTexture* > TextureMap ;
	...
	for( uint32_t i = 0, e = mTypes.Count() ; i < e ; i ++ )
	{
		...
		if( dt.LifeTime || r_decoration_quality->GetInt() > 1 )
		{
			if( dt.NormalTex )
...
DecalChief::Draw()
{
	...
	int eql = r_decoration_quality->GetInt() ;
...
DecalChief::AutoLoadTextures( DecalType& type )
{
	...
	{			
		...
		if( type.DiffuseTex->Instances == 1 )
		{
			if( !type.LifeTime )
			{

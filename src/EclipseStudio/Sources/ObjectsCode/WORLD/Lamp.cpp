...
{ 
	...
	while ( ! script.EndOfFile() )
	{
		...
		{
			LightPreset * pPreset = new LightPreset;
...
float	obj_LightHelper :: DrawPropertyEditor(float scrx, float scry, float scrw, float scrh, const AClass* startClass, const GameObjects& selected)
{
	...
	if( IsParentOrEqual( &ClassData, startClass ) )
	{
			...
			)
		{
			if( selected.Count() <= 1 )
			{
				int prevFrozen = LT.bFrozenShadowDepth ;

				starty += imgui_Checkbox( scrx, starty, "Freeze Shadow", &LT.bFrozenShadowDepth, 1 ) ;

				if( LT.bFrozenShadowDepth )
				{
				...
				if( prevFrozen != LT.bFrozenShadowDepth )
				{
					LT.UpdateDepthMap();
...
#define PROJ_TEXTURES_FOLDER "Data\\ProjectionTextures\\"
		...
		if ( imgui_Button ( scrx + 220, starty, 70, 24, "Add" ) )
		{
			...
			{
				LightPreset * pPreset = new LightPreset( preset_edit );
				pPreset->SetName( buffer );
		...
		case R3D_SPOT_LIGHT:
			{
				...
				r3dMeshSetVSConsts( final, NULL, NULL );
		...
		case R3D_TUBE_LIGHT:
			{
				...
				r3dMeshSetVSConsts( wm, NULL, NULL );
				...
				r3dMeshSetVSConsts( tmat, NULL, NULL );
...
#define PUGI_GET_IF_SET(var,name,type) attrib = lightNode.attribute(name); if( !attrib.empty() ) { var = attrib.as_##type(); }
	...
	PUGI_GET_IF_SET( LT.bFrozenShadowDepth,		"frozen_shadows"			, int	);
...
void obj_LightHelper::WriteSerializedData(pugi::xml_node& node)
{
	...
	lightNode.append_attribute("frozen_shadows")		= LT.bFrozenShadowDepth;

...
void MouseMoveUpdate()
{
	...
	if ( imgui_lbp && ! g_pHoldObject && snap_res.pObj && snap_res.pObj == g_Manipulator3d.PickedObject() )
	{
		...
		if ( g_bLockPoint )
			snap_res.pObj->ObjFlags |= OBJFLAG_SkipCastRay;
	}
	else if ( imgui_lbr )
	{
		...
		if ( g_pHoldObject )
			g_pHoldObject->ObjFlags &= ~OBJFLAG_SkipCastRay;
		g_pHoldObject = NULL;
	}
...
r3dVector ObjectManipulator3d::Picker_Move ( GameObject * pObj, POINT PickPoint, POINT pt2 )
{
	...
	r3dVector vDeltaPos = r3dVector ( 0,0,0 );

	if ( m_bAttachMode || !ObjectEditMove )
	{
		r3dVector vHoldPos = m_bAttachMode ? g_vOffsetHoldPos + pObj->GetPosition() : GetSelectionCenter();
...
void ObjectManipulator3d::Picker_Pick ( GameObject * pObj, bool bSingle )
{
	if(pObj == NULL)
		return;
	if (pObj->ObjFlags & OBJFLAG_Removed)
	...
	if ( bSingle )
	{
		for ( uint32_t i = 0; i < m_dPickedObjects.Count(); i++ )
		{
			if ( !m_dPickedObjects[i].bOldSkipCastRay )
				m_dPickedObjects[i].pObject->ObjFlags &= ~OBJFLAG_SkipCastRay;
		}
	...
	//	tInfo.pObject->ObjFlags |= OBJFLAG_SkipCastRay;


	m_dPickedObjects.PushBack(tInfo);

	m_ePickState = PICK_SELECTED;
}

void ObjectManipulator3d::PickByName	( const char* Name )
{
...
void ObjectManipulator3d::PickerAddToPicked ( GameObject * pObj )
{
	Picker_Pick ( pObj, false );
}
...
bool ObjectManipulator3d::MouseMove ( POINT pt, int delta )
{
	...
	case PICK_ELEMENT_PICKED:
		{
			if ( m_dPickedObjects.Count () == 0 )
				return false;
			r3dPoint3D vLastPos = m_dPickedObjects[0].pObject->GetPosition ();
			r3dPoint3D vLastPosCenter = GetSelectionCenter ();
			D3DXMATRIX mLastRotation = m_dPickedObjects[0].pObject->GetRotationMatrix ();
			r3dPoint3D vLastRotationVector = m_dPickedObjects[0].pObject->GetRotationVector ();
			D3DXMATRIX mLastRotationInv; 
			D3DXMatrixInverse ( &mLastRotationInv, NULL, &mLastRotation );
			if ( Picker_MoveAxis ( m_dPickedObjects[0].pObject, m_ePickElementSelected, m_PickPoint, pt, m_bLocal && m_bLocalEnable ) )
			{
				r3dPoint3D vOffset = m_dPickedObjects[0].pObject->GetPosition () - vLastPos;
				D3DXMATRIX mRotation = m_dPickedObjects[0].pObject->GetRotationMatrix ();

				D3DXMATRIX mRotAddit = mLastRotationInv * mRotation;

				m_dPickedObjects[0].pObject->SetPosition(vLastPos);
				m_dPickedObjects[0].pObject->SetRotationVector ( vLastRotationVector );
	...
	case PICK_MOVE:
		{
			r3d_assert ( m_dPickedObjects.Count () > 0 );
			r3dVector vMove = Picker_Move ( m_dPickedObjects[0].pObject, m_PickPoint, pt );
			m_PickPoint = pt;
			return true;
		}
		break;
...
void ObjectManipulator3d::ObjectActionEnd ( ControlElement_e eElem )
{
	...
	for ( uint32_t i = 0; i < m_dPickedObjects.Count(); i++ )
	{
		if ( !m_dPickedObjects[i].bOldSkipCastRay )
			m_dPickedObjects[i].pObject->ObjFlags &= ~OBJFLAG_SkipCastRay;
		m_dPickedObjects[i].pObject->OnPickerActionEnd();
	}
